...
export function ProductOrdersPage() {
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader title="Product orders" description="GET /api/admin/product-orders" />
      <p className="text-sm text-gray-600">
        <a href={legacyAdminHref("/admin/ecommerce/orders")} className="font-medium text-gray-900 underline">
          Legacy order detail →
        </a>
      </p>
      <AdminPanel>
        ...
        {q.data?.summary ? (
          <p className="mt-2 text-sm text-gray-600">{JSON.stringify(q.data.summary)}</p>
        ) : null}
      </AdminPanel>
      {rows.length === 0 ? (
        ...
        <AdminDataTable>
          ...
          <AdminTableBody>
            {rows.map((o) => (
              <tr key={String(o.id)}>
                <AdminTd className="font-mono text-xs">{String(o.order_number ?? o.id)}</AdminTd>
                <AdminTd>{String(o.customer?.full_name ?? "")}</AdminTd>
