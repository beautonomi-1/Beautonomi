...
export function CpSafetyLogsPage() {
  ...
  return (
    <div className="space-y-6">
      <CpBack />
      <AdminPageHeader title="Safety logs" description="Panic, check-in, escalation events." />
      <label className="flex flex-wrap items-center gap-2 text-sm">
        <span className="font-medium text-gray-700">Event type</span>
        <select
          className="rounded-lg border border-gray-200 px-2 py-1.5 text-sm"
          value={eventType || "all"}
          onChange={(e) => setEventType(e.target.value === "all" ? "" : e.target.value)}
        >
        ...
        <AdminPanel>
          ...
          <AdminDataTable>
            ...
            <AdminTableBody>
              {rows.map((r) => (
                <tr key={r.id}>
                  <AdminTd>{r.event_type}</AdminTd>
                  <AdminTd>{r.status}</AdminTd>
                  <AdminTd className="font-mono text-xs">{r.user_id.slice(0, 8)}…</AdminTd>
