...
export async function prepareMfaStepAfterPassword(supabase: SupabaseClient): Promise<MfaAfterPasswordResult> {
  ...
  const { data: factors, error: lfErr } = await mfa.listFactors();
  if (lfErr) return { kind: "ok" };

  const totpVerified = factors?.totp?.filter((f) => f.status === "verified") ?? [];
  ...
  if (needsAal2 && totp) {
    ...
    return { kind: "totp", factorId: totp.id, challengeId: ch.id };
  }
