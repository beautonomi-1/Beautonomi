...
export function BookingsClient({
  ...
  const [isLoading, setIsLoading] = useState(
    () => initialError === null && initialBookings === null,
  );
  ...
  useEffect(() => {
    if (initialBookings !== null && initialError === null) {
      bookingsCacheRef.current.set("all", {
...
const BookingsList = React.memo(function BookingsList({
  bookings,
  onStatusChange,
  selectedIds,
  onSelectionChange,
...
const BookingCard = React.memo(function BookingCard({
  ...
  const router = useRouter();
  const onViewDetails = useCallback(() => router.push(`/provider/bookings/${bookingId}`), [router, bookingId]);
  const handleToggle = useCallback(() => onToggleSelection?.(bookingId), [onToggleSelection, bookingId]);
  const getStatusIcon = () => {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-3 border-t border-gray-100">
          <div>
            <p className="text-lg sm:text-xl font-semibold text-gray-900">
              {booking.currency || "ZAR"} {booking.total_amount?.toFixed(2) ?? "0.00"}
            </p>
