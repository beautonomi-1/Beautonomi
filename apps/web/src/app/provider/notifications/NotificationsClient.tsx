...
const formatTimeAgo = (timestamp: string, t: (key: string, opts?: Record<string, unknown>) => string) => {
  const now = new Date();
...
export function NotificationsClient({
  ...
  const handleDeleteNotification = async (
    ...
    try {
      ...
      deleteFetcherGetCacheEntriesMatching("/api/provider/notifications");
      toast.success(t("web.provider.pages.notifications.deleted"));
    } catch (error) {
  ...
  const handleMarkUnread = async (e: React.MouseEvent, notification: Notification) => {
    ...
    try {
      ...
      setTotalUnread(prevUnread);
      toast.error(t("web.provider.pages.notifications.markUnreadFailed"));
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("web.provider.notificationsDropdown.title")}
        subtitle={t("web.provider.pages.notifications.subtitle")}
        breadcrumbs={[
          { label: t("web.provider.common.breadcrumbHome"), href: "/" },
          { label: t("web.provider.common.breadcrumbProvider"), href: "/provider" },
          { label: t("web.provider.notificationsDropdown.title") },
        ]}
      />
      ...
      <SectionCard>
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          >
            {t("web.provider.common.unread")}
          </button>
        </div>
        {isLoading ? (
          <div className="py-12">
            <LoadingTimeout loadingMessage={t("web.provider.notificationsDropdown.loading")} />
          </div>
        ) : notifications.length === 0 ? (
          <div className="py-12 text-center text-gray-500">
            <CheckCircle2 className="w-12 h-12 mx-auto mb-4 text-gray-400" />
            <p className="text-base font-medium mb-2">
              {filter === "unread" ? t("web.provider.pages.notifications.noUnread") : t("web.provider.notificationsDropdown.empty")}
            </p>
            <Link
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-sm text-gray-600">
                  {totalUnread > 0 ? t("web.provider.pages.notifications.unreadCount", { count: totalUnread }) : t("web.provider.pages.notifications.allCaughtUp")}
                </p>
                <p className="mt-1 text-xs text-gray-500">
                  {t("web.provider.pages.notifications.listHint")}
                </p>
              </div>
              {totalUnread > 0 && (
                  ...
                  <div key={notification.id}>
                    ...
                    >
                      <button
                        ...
                        className={cn(
                          "min-w-0 flex-1 text-start p-4 hover:bg-gray-50/80 transition-colors",
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-xs text-gray-500">{formatTimeAgo(notification.timestamp, t)}</span>
                              {!derivedUrl && notification.message.length > 120 && (
                                <span className="text-xs text-gray-400">
                                  ...
                                  <span className="ms-0.5">{isExpanded ? t("web.provider.common.less") : t("web.provider.common.more")}</span>
                                </span>
                              )}
                            </div>
                          </div>
                      ...
                      <div className="flex flex-col justify-start gap-0.5 py-2 pe-2 shrink-0 border-s border-gray-100 bg-white/50">
                        {notification.read && (
                          <button
                            type="button"
                            ...
                            title={t("web.provider.pages.notifications.markAsUnread")}
                          >
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-3 pt-0 ms-12 text-sm text-gray-600 space-y-2 border-b">
