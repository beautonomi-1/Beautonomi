...
/**
 * Customer SumSub verification embed – for mobile app WebView.
 *
 * No auth required: token and refresh_token are passed in the URL hash
 * so they are never sent to the server.
 *
 * Refresh callback hits /api/me/verification/sumsub/refresh (customer-specific).
 */
...
export default function CustomerSumsubEmbedPage() {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      const w = window as Window & {
        snsWebSdk?: { init: (token: string, refreshCb: () => Promise<string>) => void };
      };
      ...
      const refreshCb = (): Promise<string> => {
        ...
        return fetch(
          `${window.location.origin}/api/me/verification/sumsub/refresh?refresh_token=${encodeURIComponent(refresh_token)}`
        )
          .then((r) => r.json())
          .then((d) => d?.data?.access_token ?? d?.access_token ?? "")
          .catch(() => token);
      };
      try {
        ...
        setError("Could not start verification. Close this window and try again from the app.");
        setStatus("error");
      }
    };
    script.onerror = () => {
      ...
      {error && (
        ...
        >
          ...
          <p style={{ color: "#6b7280", fontSize: 14, marginTop: 8 }}>
            Close this window and try again from the app.
          </p>
        </div>
      )}
    </div>
