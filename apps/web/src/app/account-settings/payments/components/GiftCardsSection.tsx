...
import { supportTicketQuery, formatMoney } from "@beautonomi/utils";
...
function GiftCardsSection() {
  ...
  const formatCurrency = (amount: number, currency: string | null | undefined) =>
    formatMoney(
      Number(amount || 0),
      (currency || tenantCurrency || LAST_RESORT_CURRENCY).toUpperCase(),
    );
  ...
  const handleCopy = async (card: GiftCard) => {
    ...
    if (ok) {
      setCopiedId(card.id);
      toast.success(t("web.accountSettings.giftCards.codeCopied"));
      setTimeout(() => setCopiedId((current) => (current === card.id ? null : current)), 2000);
    } else {
      toast.error(t("web.accountSettings.giftCards.copyFailed"));
  ...
  const handleResend = async (card: GiftCard) => {
    ...
    try {
      ...
      toast.success(t("web.accountSettings.giftCards.resent"));
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("web.accountSettings.giftCards.resendFailed"));
    } finally {
      setResendingId(null);
    }
  };
  ...
  const handleUseAtBooking = async (card: GiftCard) => {
    ...
    toast.success(t("web.accountSettings.giftCards.useAtBookingToast"), {
      duration: 5000,
    });
    router.push("/");
  };
  ...
  const renderStatusBadge = (status: Status) => {
    switch (status) {
      case "active":
        return <Badge className="bg-emerald-100 text-emerald-800 hover:bg-emerald-100 text-xs">{t("web.accountSettings.giftCards.statusActive")}</Badge>;
      case "partial":
        return <Badge className="bg-emerald-100 text-emerald-800 hover:bg-emerald-100 text-xs">{t("web.accountSettings.giftCards.statusPartial")}</Badge>;
      case "used":
        return <Badge className="bg-gray-100 text-gray-700 hover:bg-gray-100 text-xs">{t("web.accountSettings.giftCards.statusUsed")}</Badge>;
      case "expired":
        return <Badge className="bg-red-100 text-red-800 hover:bg-red-100 text-xs">{t("web.accountSettings.giftCards.statusExpired")}</Badge>;
      case "inactive":
        return <Badge variant="outline" className="text-xs">{t("web.accountSettings.giftCards.statusInactive")}</Badge>;
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="backdrop-blur-xl bg-white/80 border border-white/40 rounded-xl p-6 mb-6">
        <LoadingTimeout loadingMessage={t("web.accountSettings.giftCards.loading")} />
      </div>
    );
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        <div>
          <h2 className="text-xl font-semibold tracking-tighter text-gray-900">
            {t("web.accountSettings.giftCards.title")}
          </h2>
          {sortedCards.length > 0 && (
            <p className="text-sm text-gray-600 mt-1">
              {usableCount > 0
                ? usableCount === 1
                  ? t("web.accountSettings.giftCards.usableCountOne", { count: usableCount })
                  : t("web.accountSettings.giftCards.usableCountMany", { count: usableCount })
                : t("web.accountSettings.giftCards.noUsableBalance")}
            </p>
          )}
        </div>
        <Link
        ...
        >
          {t("web.accountSettings.giftCards.buyGiftCard")}
        </Link>
      </div>
      ...
      {sortedCards.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-base font-light mb-4 text-gray-600">
            {t("web.accountSettings.giftCards.empty")}
          </p>
          <Link
            href="/gift-card/purchase"
            className="inline-block bg-gradient-to-r from-primary to-primary-hover hover:from-primary-hover hover:to-primary text-white px-4 md:px-6 py-2 md:py-3 rounded-xl font-semibold text-sm md:text-base transition-all shadow-lg hover:shadow-xl text-center"
          >
            {t("web.accountSettings.giftCards.purchaseFirst")}
          </Link>
        </div>
      ) : (
        <div className="space-y-3">
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-gray-500">
                    ...
                    {t("web.accountSettings.giftCards.giftCredit")}
                  </div>
                ...
                <div className="mb-4">
                  <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                    ...
                    {initial > 0 && initial !== balance && (
                      <span className="text-sm text-gray-500">
                        {t("web.accountSettings.giftCards.ofAmount", {
                          amount: formatCurrency(initial, card.currency),
                        })}
                      </span>
                    )}
                  </div>
                  {initial > 0 && (
                    <div
                      ...
                      aria-label={t("web.accountSettings.giftCards.percentRemaining", {
                        percent: 100 - usedPct,
                      })}
                    >
                      <div
                ...
                <div className="rounded-xl border border-gray-200 bg-white p-3 sm:p-4 mb-3">
                  <div className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">
                    {t("web.accountSettings.giftCards.codeLabel")}
                  </div>
                  <div
                    className="font-mono text-base sm:text-lg font-semibold text-gray-900 break-all select-all leading-relaxed"
                    aria-label={
                      isRevealed
                        ? t("web.accountSettings.giftCards.codeAria", { code: card.code })
                        : t("web.accountSettings.giftCards.codeHiddenAria")
                    }
                  >
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    >
                      {isRevealed ? (
                        <>
                          ...
                          {t("web.accountSettings.giftCards.hide")}
                        </>
                      ) : (
                        <>
                          <Eye className="w-4 h-4" />
                          {t("web.accountSettings.giftCards.showCode")}
                        </>
                      )}
                    </button>
                    <button
                    ...
                    >
                      {isCopied ? (
                        <>
                          ...
                          {t("web.accountSettings.giftCards.copied")}
                        </>
                      ) : (
                        <>
                          <Copy className="w-4 h-4" />
                          {t("web.accountSettings.giftCards.copyCode")}
                        </>
                      )}
                    </button>
                    {usable && (
                      ...
                      >
                        {t("web.accountSettings.giftCards.useAtBooking")}
                    ...
                    {card.can_resend && usable ? (
                      ...
                      >
                        {resendingId === card.id
                          ? t("web.accountSettings.giftCards.resending")
                          : t("web.accountSettings.giftCards.resend")}
                      </button>
                    ) : null}
                    <Link
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 text-sm font-medium text-gray-700 transition-colors"
                    >
                      {t("web.accountSettings.giftCards.contactSupport")}
                    </Link>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-600">
                  {expiresAt && (
                    <span>
                      {status === "expired"
                        ? t("web.accountSettings.giftCards.expiredOn", {
                            date: expiresAt.toLocaleDateString(undefined, {
                              year: "numeric",
                              month: "short",
                              day: "numeric",
                            }),
                          })
                        : t("web.accountSettings.giftCards.expiresOn", {
                            date: expiresAt.toLocaleDateString(undefined, {
                              year: "numeric",
                              month: "short",
                              day: "numeric",
                  ...
                  {card.deliver_at && !card.delivered_at ? (
                    <span>
                      {t("web.accountSettings.giftCards.sendsOn", {
                        date: new Date(card.deliver_at).toLocaleString(undefined, {
                          year: "numeric",
                          month: "short",
                          day: "numeric",
                          hour: "2-digit",
                  ...
                  {card.delivered_at ? <span>{t("web.accountSettings.giftCards.delivered")}</span> : null}
                  {meta.sender_name && (
                    <span>{t("web.accountSettings.giftCards.fromSender", { name: meta.sender_name })}</span>
                  )}
                  {meta.recipient_name && (
                    <span>{t("web.accountSettings.giftCards.forRecipient", { name: meta.recipient_name })}</span>
                  )}
                </div>
                {meta.message && (
                  <p className="mt-2 text-sm italic text-gray-700 border-s-2 border-primary/30 ps-3">
                    “{meta.message}”
                  </p>
                )}
              </div>
          ...
          <p className="text-xs text-gray-500 mt-2">
            {t("web.accountSettings.giftCards.checkoutHint")}
          </p>
        </div>
      )}
    </div>
