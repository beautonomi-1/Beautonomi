...
const updateSchema = z.object({
  ...
  delivery_fee_type: z.enum(["flat", "weight_based", "distance_based"]).optional(),
  free_delivery_threshold: z.number().min(0).nullable().optional(),
  delivery_radius_km: z.number().min(0).nullable().optional(),
  weight_rate_per_kg: z.number().min(0).nullable().optional(),
  distance_rate_per_km: z.number().min(0).nullable().optional(),
  shipping_provider_preference: z.enum(["aramex", "courier-guy", "bob-go"]).nullable().optional(),
