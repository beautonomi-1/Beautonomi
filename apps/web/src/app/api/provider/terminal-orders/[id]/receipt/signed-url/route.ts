/**
 * POST /api/provider/terminal-orders/[id]/receipt/signed-url
 *
 * Mint a short-lived HMAC-signed URL so the native provider app can open
 * the terminal order receipt PDF directly (no Bearer on the final GET).
 */
...
export async function POST(
  ...
  try {
    ...
    const url = `${origin}/api/provider/terminal-orders/${encodeURIComponent(id)}/receipt/pdf?token=${encodeURIComponent(token)}`;
    const expiresAt = new Date(Date.now() + ttlSeconds * 1000).toISOString();

    return successResponse({ url, expires_at: expiresAt });
  } catch (error) {
    return handleApiError(error, "Failed to mint terminal order receipt download URL");
  }
}
