...
export async function validateBooking(
  ...
  if (isGroupBooking && groupParticipants && groupParticipants.length > 0) {
    ...
    if (allStaffNull && draft.provider_id) {
      ...
      const providerTzForAnyStaffPick =
        normalizeProviderTimezone((provider as { timezone?: string | null }).timezone) ?? DEFAULT_BOOKING_DISPLAY_TIMEZONE;
      const travelBufferMinutesForAnyStaffPick =
        draft.location_type === "at_home"
          ? Number(validatedDraft.availability_travel_buffer_minutes ?? 0)
          : 0;
      const picked = await pickFirstStaffForNullStaffLines({
        ...
        providerTimeZone: providerTzForAnyStaffPick,
        travelBufferMinutes: travelBufferMinutesForAnyStaffPick,
  ...
  // staff_schedules → working_hours → location hours, then clamp to location window.
  {
    ...
    // the listing engine (calculate-slots.ts / shiftMinuteRanges).
    const segmentFitsAtLeastOneShift = segmentFitsAnyShift;

    // Slots in the early morning (before 08:00 provider-local) may have been
    // generated by an overnight shift that started on the previous calendar day
    // (e.g. a 22:00–02:00 shift).  `formatInTimeZone` maps the instant to
    // the CURRENT calendar day but `loadEffectiveStaffShifts` only returns shifts
    // configured for that day — which may not include the previous day's overnight
    // shift.  When the same-day check fails AND the slot is before
    // OVERNIGHT_MORNING_CUTOFF_MIN, we also check whether any overnight shift
    // from the previous calendar day covers the segment.
    const OVERNIGHT_MORNING_CUTOFF_MIN = 8 * 60; // 08:00
    ...
    const getShiftsForDate = async (staffId: string, date: string) => {
      ...
      let r = shiftCache.get(key);
      if (!r) {
        r = await loadEffectiveStaffShifts(
          supabaseAdmin,
          staffId,
          date,
          draft.provider_id,
    ...
    for (let i = 0; i < bookingServicesData.length; i++) {
      ...
      const { staffShifts, workHoursEnabledEffective } = await getShiftsForDate(staffIdForGuard, localDate);

      const segStartMin = minutesFromInstantInZone(segStart);
      const segEndMin = minutesFromInstantInZone(effectiveSegEnd);

      // Primary check: does the segment fit any shift on its own calendar day?
      let fitsShifts = workHoursEnabledEffective &&
        staffShifts.length > 0 &&
        segmentFitsAtLeastOneShift(segStartMin, segEndMin, staffShifts);

      // Overnight fallback: if the same-day check failed and the segment starts
      // before 08:00 provider-local, check whether an overnight shift from the
      // previous calendar day covers it.  The listing engine generates the slot
      // as dayOffset=1 of the previous day's overnight shift, but stores the ISO
      // instant with the correct next-day date — `localDate` ends up being the
      // continuation day, not the shift definition day.
      if (!fitsShifts && segStartMin < OVERNIGHT_MORNING_CUTOFF_MIN) {
        ...
        const prevResolved = await getShiftsForDate(staffIdForGuard, prevDate);
        if (prevResolved.workHoursEnabledEffective && prevResolved.staffShifts.length > 0) {
          // Only accept as overnight-continuation if the prev-day shift is actually
          // an overnight shift (end_time < start_time or end_time = "00:00").
          const overnightShifts = prevResolved.staffShifts.filter((s) => {
            ...
            return endMin < startMin || (s.end_time === "00:00" || s.end_time === "00:00:00");
          });
          if (overnightShifts.length > 0) {
            fitsShifts = segmentFitsAtLeastOneShift(segStartMin, segEndMin, overnightShifts);
            if (fitsShifts) {
              console.info(
                `[validateBooking] shift-guard: overnight fallback matched for ${segStart.toISOString()} ` +
                  `(prev-day ${prevDate} overnight shifts)`,
              );
            }
          }
        }
      ...
      if (!fitsShifts) {
        const noShiftsAtAll = staffShifts.length === 0 || !workHoursEnabledEffective;
        console.warn(
          `[validateBooking] shift-guard: segment ${segStart.toISOString()}–${effectiveSegEnd.toISOString()} ` +
            ...
            (noShiftsAtAll ? " (no shifts configured)" : ""),
        );
        // Distinguish "no hours configured" (schedule data issue) from "slot outside hours" so
        // callers can surface the right message — both result in a CONFLICT 409 to the client.
        return handleApiError(
          new Error(
            noShiftsAtAll
              ? "No working hours are configured for this time slot"
              : "Booking falls outside working hours",
          ),
          "This time is no longer available. Please go back and select a different time.",
          "CONFLICT",
          409,
        );
      }
