...
/**
 * GET /api/me/orders/[id]/receipt/pdf
 *
 * Auth modes (mirrors booking receipt PDF):
 *   1. Normal `Authorization: Bearer` / cookie session (customer web).
 *   2. Short-lived `?token=<hmac>` minted by
 *      POST /api/me/orders/[id]/receipt/signed-url (native customer app).
 */
...
export async function GET(
  ...
  try {
    ...
    const upstream = await getOrderReceiptJson(request, {
    ...
    const payload = (await upstream.json()) as OrderReceiptPayload;
