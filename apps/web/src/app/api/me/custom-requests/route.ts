    ...
    const { data, error } = await supabase
      ...
      .select(
        ...
        *,
        ...
        offers:custom_offers(id, price, currency, duration_minutes, expiration_at, notes, status, payment_url, payment_reference, paid_at, created_at, staff_id, location_id, scheduled_at, travel_fee, change_request_note, changes_requested_at, staff:provider_staff(id, name), location:provider_locations(id, name))
      `
      )
      .eq("customer_id", user.id)
