...
import { getWasenderConfig, connectSession } from "@/lib/whatsapp/wasender-client";
...
export async function POST(
  ...
  try {
    ...
    const result = await connectSession(config, (session as any).wasender_session_id);

    await supabase
      .from("whatsapp_sessions")
      .update({ status: "connecting", last_status_check_at: new Date().toISOString() })
      .eq("id", id);

    return successResponse(result);
  } catch (error) {
    return handleApiError(error, "Failed to connect session");
  }
}
