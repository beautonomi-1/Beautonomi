 ...
 *
 * Kept in sync with `apps/admin-web/src/lib/cmsPageSectionPresets.ts` (admin SPA + legacy Next admin).
 */
...
const CMS_PAGE_SLUG_TITLES: Record<string, string> = {
  ...
  "provider-eula": "Partner EULA",
  "customer-eula": "Customer EULA",
...
export function cmsPagePublicApiHint(pageSlug: string): string | null {
  ...
  if (
    ...
    pageSlug === "provider-eula" ||
    pageSlug === "customer-eula" ||
