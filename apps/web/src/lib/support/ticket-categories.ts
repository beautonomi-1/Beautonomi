/**
 * Support ticket categories for user-submitted tickets (Help → Submit ticket).
 * Values are stored on support_tickets.category — keep stable for reporting/filters.
 */
...
export const SUPPORT_TICKET_CATEGORY_VALUES = _values;

export function isValidSupportTicketCategory(value: string | undefined | null): boolean {
  if (!value || typeof value !== "string") return false;
  return SUPPORT_TICKET_CATEGORY_VALUES.has(value.trim());
}

/** Normalizes API input; falls back for legacy clients. */
export function normalizeSupportTicketCategory(value: unknown): string {
  if (typeof value !== "string") return "general_inquiry";
  const t = value.trim();
  if (isValidSupportTicketCategory(t)) return t;
