...
function formatRelativeUpdate(
  ...
  if (delta < 5) return t("web.provider.bookings.liveSync.justNow");
  if (delta < 60) return t("web.provider.bookings.liveSync.secondsAgo", { count: delta });
  return t("web.provider.bookings.liveSync.minutesAgo", { count: Math.floor(delta / 60) });
}
...
export function BookingLiveSyncIndicator({ lastUpdatedAt, className }: BookingLiveSyncIndicatorProps) {
  ...
  return (
    ...
    >
      ...
      {t("web.provider.bookings.liveSync.label", {
        relative: formatRelativeUpdate(lastUpdatedAt, t),
      })}
    </div>
  );
}
