...
export default function Breadcrumb({ items }: BreadcrumbProps) {
  ...
  return (
    <nav className="mb-4 md:mb-6 w-full max-w-full overflow-hidden" aria-label="Breadcrumb">
      <ol className="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm text-gray-500 overflow-x-auto scrollbar-hide w-full max-w-full box-border">
        {displayItems.map((item, displayIndex) => {
          ...
          return (
            <li key={originalIndex} className="flex items-center flex-shrink-0 min-w-0">
              {shouldShowEllipsis && displayIndex === 1 && (
                <>
                  <span className="mx-1 md:mx-2 text-gray-400 flex-shrink-0">...</span>
              ...
              {item.href && !isLast ? (
                ...
                <span className={cn(
                  isLast ? "text-gray-900 font-medium px-1" : "px-1",
