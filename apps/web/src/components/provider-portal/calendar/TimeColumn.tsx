...
function TimeColumnComponent({ timeSlots }: TimeColumnProps) {
  return (
    <div
      className="flex-shrink-0 border-e-2 border-gray-200 bg-gray-50/80 sticky left-0 z-10"
      style={{ width: `${TIME_COLUMN_WIDTH}px` }}
    >
      {timeSlots.map((time) => (
        <div
          key={time}
          className="border-b border-gray-200 flex items-start justify-end pe-2 pt-0"
