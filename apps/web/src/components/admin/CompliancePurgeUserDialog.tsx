...
const PHRASE = "DELETE USER FOREVER";
...
export function CompliancePurgeUserDialog(props: {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  userId: string;
  userEmail: string;
  /** Called when the user closes the dialog after a successful purge (from the report step). */
  onComplete?: () => void;
}) {
  const { open, onOpenChange, userId, userEmail, onComplete } = props;
  ...
  const submit = async () => {
    ...
    try {
      setBusy(true);
      const res = (await fetcher.post("/api/admin/compliance/purge-user", {
        user_id: userId,
        reason: reason.trim(),
      ...
      toast.success("Account purged. Save the confirmation report if required.");
    } catch (err) {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
        {!done ? (
          <>
            <DialogHeader>
              <DialogTitle>Purge user (compliance)</DialogTitle>
              <DialogDescription className="text-left space-y-2">
                <span className="block">
                  Permanently deletes this login, profile, and data that cascade from the user. For
                  regulatory or verified erasure requests only.
                </span>
            ...
            <div className="space-y-3">
              <div>
                <Label htmlFor="purge-reason">Reason (min. 20 characters)</Label>
                <Textarea
                  id="purge-reason"
                  value={reason}
              ...
              <div>
                <Label htmlFor="purge-email">Type the account email to confirm</Label>
                <Input
                  id="purge-email"
                  value={emailConfirm}
                ...
                />
                <p className="mt-1 text-xs text-muted-foreground">Must match: {userEmail}</p>
              </div>
              <div>
                <Label htmlFor="purge-phrase">Confirmation phrase</Label>
                <Input
                  id="purge-phrase"
                  value={phrase}
              ...
              <div className="flex items-start gap-2">
                ...
                />
                <label htmlFor="purge-ack" className="text-sm leading-tight text-muted-foreground">
                  I understand this permanently removes the account and associated platform data, and I am
