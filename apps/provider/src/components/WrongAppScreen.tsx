...
export function WrongAppScreen({ portal, onSignOut }: WrongAppScreenProps) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.white, paddingHorizontal: 24, justifyContent: "center", alignItems: "center" }}>
      <Text style={{ fontSize: 20, fontWeight: "600", color: Colors.gray[900], textAlign: "center", marginBottom: 8 }}>
        Wrong app
      </Text>
      <Text style={{ fontSize: 16, color: Colors.gray[600], textAlign: "center", marginBottom: 24 }}>
        {isCustomer &&
          "This account is for customers. Use the Customer app to book services."}
        {isAdmin &&
      ...
      {isAdmin && APP_URL ? (
        <Pressable
          onPress={() =>
            Linking.openURL(`${APP_URL.replace(/\/$/, "")}/admin/dashboard`)
          }
