...
export default function MarketAvailabilityGate() {
  ...
  const ma = (key: string, options?: Record<string, string>) =>
    (options != null
      ? t(`provider.mobile.components.marketAvailability.${key}`, options as never)
      : t(`provider.mobile.components.marketAvailability.${key}`)) as string;
  ...
  const restrictedDetails = reason || ma("restrictedDetailsFallback");
        ...
        ? bannerModal(
            <View style={{ backgroundColor: "#0c4a6e", borderRadius: 14, padding: 16 }}>
              ...
              <Text style={{ color: "#bae6fd", marginBottom: 14, lineHeight: 20 }}>
                {ma("regionalBody", {
                  host: defaultMarketHost,
                  suffix: countryCode ? ma("regionalForeignSuffix", { country: countryCode }) : "",
                })}
              </Text>
              <View style={{ gap: 10 }}>
                <Pressable
