...
export type E164PhoneFieldProps = {
  /** Stored / API value (E.164). Empty = no number. */
  valueE164: string;
  onChangeE164: (e164: string) => void;
  label?: string;
  /**
   * When the field is empty, this dial code is used (flag + parsing). Defaults to device locale, or +27 in hook callers.
   */
  defaultCountryDial?: string;
  placeholderNational?: string;
  /** Smaller padding (e.g. walk-in bottom sheet). */
  compact?: boolean;
  /** Gray background to match booking / group forms. */
  muted?: boolean;
  /** Short hint under the field (set false to hide). */
  showHint?: boolean;
...
export function E164PhoneField({
  ...
  const pi = (key: string) => t(`provider.mobile.components.phoneInput.${key}`) as string;
  ...
  const nationalPlaceholder =
    placeholderNational ??
    (resolvedDefaultDial === "+27" ? "82 123 4567" : pi("mobileNumberPlaceholder"));
  ...
  return (
    <View style={label ? twStyle("mb-3") : undefined}>
      {label ? (
      ...
      {fieldError ? <Text style={twStyle("mt-1 text-xs text-red-500")}>{fieldError}</Text> : null}
              ...
              renderItem={({ item: c }: { item: CountryCodeOption }) => (
                <TouchableOpacity
                  onPress={() => {
                    ...
                    setShowPicker(false);
                    setFieldError(
                      national.trim() ? validateNationalPhoneDigits(national, c.code) : null,
                    );
