...
const BASIS_KEYS: Record<string, string> = {
  ...
  profit: "basisProfit",
  topProducts: "basisTopProducts",
  averageRevenuePerUnit: "basisAverageRevenuePerUnit",
};
...
export function ProductSalesReportView({ data }: { data: unknown }) {
  ...
  const ps = useCallback(
    (key: string, opts?: Record<string, unknown>) =>
      t("provider.mobile.screens.productSalesReport." + key, opts) as string,
    [t],
  );
    ...
    <View style={twStyle("gap-5 pb-8")}>
      ...
      <View style={twStyle("flex-row flex-wrap gap-3")}>
        <View style={twStyle("min-w-[140px] flex-1 rounded-2xl border border-gray-100 bg-white px-4 py-3")}>
          <Text style={twStyle("text-xs font-medium text-gray-600")}>{ps("unitsSold")}</Text>
        ...
        <View style={twStyle("min-w-[140px] flex-1 rounded-2xl border border-emerald-100 bg-emerald-50/90 px-4 py-3")}>
          <Text style={twStyle("text-xs font-medium text-emerald-900")}>{ps("totalRevenue")}</Text>
      ...
      <View style={twStyle("flex-row flex-wrap gap-3")}>
        <View style={twStyle("min-w-[140px] flex-1 rounded-2xl border border-indigo-100 bg-indigo-50/90 px-4 py-3")}>
          <Text style={twStyle("text-xs font-medium text-indigo-900")}>{ps("totalProfit")}</Text>
        ...
        <View style={twStyle("min-w-[140px] flex-1 rounded-2xl border border-slate-100 bg-slate-50/90 px-4 py-3")}>
          <Text style={twStyle("text-xs font-medium text-slate-900")}>{ps("avgRevenuePerUnit")}</Text>
        ...
        <>
          ...
          <View style={twStyle("flex-row flex-wrap gap-3")}>
            <View style={twStyle("min-w-[140px] flex-1 rounded-2xl border border-amber-100 bg-amber-50/85 px-4 py-3")}>
              <Text style={twStyle("text-xs font-medium text-amber-950")}>{ps("appointmentAddons")}</Text>
              <Text style={twStyle("mt-1 text-xs leading-4 text-amber-950/85")}>
                {ps("windowScheduledAt")}
              </Text>
              ...
              <Text style={twStyle("text-xs text-amber-950/80")}>
                {ps("unitsCount", { count: Number(data.unitsFromBookings ?? 0) })}
              </Text>
            </View>
            <View style={twStyle("min-w-[140px] flex-1 rounded-2xl border border-teal-100 bg-teal-50/85 px-4 py-3")}>
              <Text style={twStyle("text-xs font-medium text-teal-950")}>{ps("paidRetailOrders")}</Text>
              ...
              <Text style={twStyle("text-xs text-teal-950/80")}>
                {ps("unitsCount", { count: Number(data.unitsFromOrders ?? 0) })}
              </Text>
            </View>
          </View>
          {data.by_channel ? (
            <View style={twStyle("flex-row flex-wrap gap-3")}>
              <View style={twStyle("min-w-[140px] flex-1 rounded-2xl border border-blue-100 bg-blue-50/85 px-4 py-3")}>
                <Text style={twStyle("text-xs font-medium text-blue-950")}>{ps("onlineRetail")}</Text>
                ...
                <Text style={twStyle("text-xs text-blue-950/80")}>
                  {ps("unitsCount", { count: Number(data.by_channel.online?.units ?? 0) })}
                </Text>
              </View>
              <View style={twStyle("min-w-[140px] flex-1 rounded-2xl border border-orange-100 bg-orange-50/85 px-4 py-3")}>
                <Text style={twStyle("text-xs font-medium text-orange-950")}>{ps("walkInRetail")}</Text>
                ...
                <Text style={twStyle("text-xs text-orange-950/80")}>
                  {ps("unitsCount", { count: Number(data.by_channel.walk_in?.units ?? 0) })}
                </Text>
              </View>
            </View>
          ) : null}
      ...
      <Text style={twStyle("text-xs font-semibold uppercase tracking-wide text-gray-500")}>{ps("topProducts")}</Text>
          ...
          <View style={twStyle("flex-1 pe-3")}>
            <View style={twStyle("flex-row items-center gap-2")}>
              ...
              <Text style={twStyle("flex-1 font-medium text-gray-900")} numberOfLines={2}>
                {p.productName ?? ps("productFallback")}
              </Text>
            </View>
            <Text style={twStyle("mt-1 ps-10 text-xs text-gray-500")}>
              {typeof p.averagePrice === "number"
                ? ps("soldAvg", { count: Number(p.quantitySold ?? 0), amount: formatCurrency(p.averagePrice) })
                : ps("soldCount", { count: Number(p.quantitySold ?? 0) })}
            </Text>
          </View>
          <View style={twStyle("items-end")}>
            ...
            {p.profit != null ? (
              <Text style={twStyle("text-xs text-gray-500")}>
                {ps("profitAmount", { amount: formatCurrency(Number(p.profit)) })}
              </Text>
            ) : null}
          </View>
        </View>
            ...
            >
              <View>
                <Text style={twStyle("font-medium capitalize text-gray-900")}>
                  {c.category || ps("uncategorized")}
                </Text>
                <Text style={twStyle("text-xs text-gray-500")}>{ps("soldCount", { count: Number(c.quantitySold ?? 0) })}</Text>
              </View>
              <View style={twStyle("items-end")}>
                <Text style={twStyle("font-semibold tabular-nums text-gray-900")}>
                ...
                {c.profit != null ? (
                  <Text style={twStyle("text-xs text-gray-500")}>
                    {ps("profitAmount", { amount: formatCurrency(Number(c.profit)) })}
                  </Text>
                ) : null}
              </View>
            </View>
