...
export function ClientSummaryReportView({ data }: { data: unknown }) {
  ...
  const cs = useCallback(
    (key: string, opts?: Record<string, unknown>) =>
      t("provider.mobile.screens.clientSummaryReport." + key, opts) as string,
    [t],
  );
    ...
    <View style={twStyle("gap-5 pb-8")}>
      ...
      {basis ? (
        <View style={twStyle("rounded-2xl border border-sky-100 bg-sky-50/95 px-4 py-3")}>
          ...
          {data.timezone ? (
            <Text style={twStyle("mt-2 text-xs text-sky-900/85")}>{cs("timezone", { timezone: data.timezone })}</Text>
          ) : null}
        </View>
      ) : null}
      ...
      <View style={twStyle("flex-row flex-wrap gap-3")}>
        <View style={twStyle("min-w-[148px] flex-1 rounded-2xl border border-blue-100 bg-blue-50/90 px-4 py-3")}>
          <Text style={twStyle("text-xs font-medium text-blue-900")}>{cs("clientsWindow")}</Text>
        ...
        <View style={twStyle("min-w-[148px] flex-1 rounded-2xl border border-emerald-100 bg-emerald-50/90 px-4 py-3")}>
          <Text style={twStyle("text-xs font-medium text-emerald-900")}>{cs("newFirstInScope")}</Text>
        ...
        <View style={twStyle("min-w-[148px] flex-1 rounded-2xl border border-indigo-100 bg-indigo-50/90 px-4 py-3")}>
          <Text style={twStyle("text-xs font-medium text-indigo-900")}>{cs("twoPlusInWindow")}</Text>
        ...
        <View style={twStyle("min-w-[148px] flex-1 rounded-2xl border border-violet-100 bg-violet-50/90 px-4 py-3")}>
          <Text style={twStyle("text-xs font-medium text-violet-900")}>{cs("avgBookedGross")}</Text>
        ...
        {data.averageLedgerEarnings != null ? (
          <View style={twStyle("min-w-[148px] flex-1 rounded-2xl border border-emerald-100 bg-emerald-50/90 px-4 py-3")}>
            <Text style={twStyle("text-xs font-medium text-emerald-900")}>{cs("avgLedgerEarnings")}</Text>
      ...
      <View style={twStyle("rounded-2xl border border-gray-100 bg-white px-4 py-3")}>
        <Text style={twStyle("text-xs font-medium text-gray-600")}>{cs("avgBookingsRetention")}</Text>
        <Text style={twStyle("mt-1 text-sm text-gray-900")}>
          {cs("avgBookingsLine", {
            bookings: avgBk.toFixed(1),
            rate: retRate != null ? formatPercentage(retRate) : empty,
            days: days != null ? cs("daysSuffix", { count: days }) : "",
          })}
        </Text>
      </View>
      ...
      <View style={twStyle("rounded-2xl border border-gray-100 bg-white")}>
        {top.length === 0 ? (
          <Text style={twStyle("px-4 py-6 text-center text-sm text-gray-500")}>{cs("noRows")}</Text>
        ) : (
          top.map((c, i) => (
            <View
            ...
            >
              <View style={twStyle("me-2 flex-1")}>
                ...
                <Text style={twStyle("text-xs text-gray-500")}>
                  {cs("bookingsCount", { count: c.totalBookings ?? 0 })}
                  {c.averageRating != null && c.averageRating > 0
                    ? cs("ratingSuffix", { rating: c.averageRating.toFixed(1) })
                    : ""}
                </Text>
              </View>
              <View style={twStyle("items-end")}>
                ...
                {c.ledgerEarnings != null && c.ledgerEarnings > 0 ? (
                  <Text style={twStyle("text-xs tabular-nums text-emerald-700")}>
                    {cs("ledgerAmount", { amount: formatCurrency(c.ledgerEarnings) })}
                  </Text>
                ) : null}
              </View>
            </View>
      ...
      {Object.keys(detailPayload).length > 0 ? (
        <ReportPayloadView data={detailPayload} title={cs("extra")} />
      ) : null}
    </View>
  );
}
