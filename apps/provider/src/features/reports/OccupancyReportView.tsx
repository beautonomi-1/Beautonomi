...
function formatPct(p: number | null | undefined, empty: string): string {
...
export function OccupancyReportView({ data }: { data: unknown }) {
  ...
  const oc = useCallback(
    (key: string, opts?: Record<string, unknown>) =>
      t("provider.mobile.screens.occupancyReport." + key, opts) as string,
    [t],
  );
    ...
    <View style={twStyle("gap-5 pb-8")}>
          ...
          <View style={twStyle("rounded-2xl border border-sky-100 bg-sky-50/95 px-4 py-3")}>
            ...
            {statuses.length > 0 ? (
              <Text style={twStyle("mt-2 text-xs leading-5 text-sky-900/90")}>
                {oc("bookingStatuses", { statuses: statuses.join(", ") })}
              </Text>
            ) : null}
            {data.timezone ? (
              <Text style={twStyle("mt-1 text-xs text-sky-900/85")}>{oc("timezone", { timezone: data.timezone })}</Text>
            ) : null}
          </View>
        ) : null}
        ...
        {summary ? (
          <View style={twStyle("flex-row flex-wrap gap-3")}>
            ...
            <View style={twStyle("min-w-[148px] flex-1 rounded-2xl border border-teal-100 bg-teal-50/90 px-4 py-3")}>
              <Text style={twStyle("text-xs font-medium text-teal-900")}>{oc("bookedMin")}</Text>
            ...
            <View style={twStyle("min-w-[148px] flex-1 rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3")}>
              <Text style={twStyle("text-xs font-medium text-gray-700")}>{oc("staffDays")}</Text>
              <Text style={twStyle("mt-1 text-xl font-semibold tabular-nums text-gray-900")}>
                {oc("staffDaysValue", {
                  staff: summary.staffMemberCount ?? empty,
                  days: summary.dayCount ?? empty,
                })}
              </Text>
            </View>
          </View>
        ...
        <View style={twStyle("rounded-2xl border border-gray-100 bg-white px-3 py-2")}>
          <Text style={twStyle("text-[11px] leading-4 text-gray-600")}>
            {oc("occupancyDashHint")}
          </Text>
        </View>

        <Text style={twStyle("text-xs font-semibold uppercase tracking-wide text-gray-500")}>{oc("byDate")}</Text>
          ...
          {byDate.map((row) => (
            ...
            >
              ...
              <Text style={twStyle("text-sm tabular-nums text-gray-700")}>
                {oc("rowAvailBooked", { avail: row.totalAvailable ?? 0, booked: row.totalBooked ?? 0 })}
                <Text style={twStyle("font-semibold text-gray-900")}>{formatPct(row.occupancyPercent ?? null, empty)}</Text>
              </Text>
            </View>
          ))}
        </View>
        ...
        {byStaff.length > 0 ? (
          <>
            <Text style={twStyle("text-xs font-semibold uppercase tracking-wide text-gray-500")}>{oc("byStaff")}</Text>
            {byStaff.map((staff) => (
                ...
                {(staff.byDate ?? []).map((row) => (
                  ...
                  >
                    ...
                    <Text style={twStyle("text-xs tabular-nums text-gray-800")}>
                      {oc("rowAvailBooked", { avail: row.availableMinutes ?? 0, booked: row.bookedMinutes ?? 0 })}
                      <Text style={twStyle("font-semibold")}>{formatPct(row.occupancyPercent ?? null, empty)}</Text>
                    </Text>
                  </View>
                ))}
              </View>
        ...
        {Object.keys(detailPayload).length > 0 ? (
          <ReportPayloadView data={detailPayload} title={oc("rawSummaryFields")} />
        ) : null}
    </View>
  );
}
