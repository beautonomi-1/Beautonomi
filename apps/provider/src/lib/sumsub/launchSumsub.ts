/**
 * Provider-side Sumsub native SDK launcher.
 *
 * Fetches a fresh access token from the provider's token endpoint, then
...
async function fetchProviderToken(env: string): Promise<string> {
  ...
  if (!token) throw new Error("Sumsub provider token endpoint returned no access_token");
  return token;
}
...
export async function launchSumsub(options: LaunchSumsubOptions): Promise<LaunchSumsubResult> {
  ...
  try {
    const initialToken = await fetchProviderToken(env);

    const tokenExpirationHandler = async (): Promise<string> => {
      try {
        return await fetchProviderToken(env);
      } catch (e) {
    ...
    console.error("[launchSumsub] provider SDK launch failed:", msg);
