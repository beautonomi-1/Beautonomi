...
import { navigateFromSafetyHub, useFromSafetyHub } from "@/lib/provider-tab-navigation";
...
export default function BlockedUsersScreen() {
  ...
  const ph = useCallback(
    (key: string, opts?: Record<string, string | number>) =>
      t(`provider.mobile.screens.blockedUsers.${key}`, opts ?? {}) as string,
    [t],
  );

  const title = t("customer.mobile.screens.blockedUsers.title");
  const breadcrumbSegment = t("provider.mobile.screens.blockedUsers.breadcrumb", {
  ...
  const openReportUser = useCallback(() => {
    const pathname = "/(app)/(tabs)/more/safety/report-user";
    if (fromSafety) {
      ...
      <View style={{ flex: 1, backgroundColor: Colors.gray[50] }}>
        {blockedUsers.length > 8 ? (
          <View style={{ paddingHorizontal: 16, paddingBottom: 8 }}>
            <TextInput
              ...
              placeholder={ph("searchPlaceholder")}
              placeholderTextColor={Colors.gray[400]}
              accessibilityLabel={ph("searchPlaceholder")}
              style={{
        ...
        {loading ? (
          ...
          <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 24 }}>
            ...
            >
              <Text style={{ color: Colors.white, fontWeight: "600" }}>{ph("emptyReportCta")}</Text>
            </TouchableOpacity>
          </View>
