/**
 * Top-anchored in-app banner for non-interruptive alerts (booking updates, messages, etc.).
 * Queues items when one is already visible; auto-dismisses after ~4s.
 * Ported from the provider app — adapted for the customer Colors palette.
 */
import {
  createContext,
  useCallback,
...
export type InAppBannerTone = "default" | "success" | "info" | "warning";
...
function toneColors(tone: InAppBannerTone) {
  switch (tone) {
    ...
    case "info":
      return { bg: "#eff6ff", border: "#bfdbfe", icon: Colors.info, text: "#1e3a8a" };
    case "warning":
      return { bg: "#fffbeb", border: "#fde68a", icon: "#d97706", text: "#92400e" };
    default:
