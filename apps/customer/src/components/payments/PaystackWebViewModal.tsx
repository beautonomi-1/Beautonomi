/**
 * In-app Paystack hosted checkout (standard / inline). Intercepts success/cancel
 * URLs so the user stays in the RN shell (no ASWebAuthenticationSession / external browser).
 */
...
export type PaystackWebViewModalProps = {
  ...
  title?: string;
  /** Return true when navigation should complete the flow (intercept and close). */
  matchSuccess: (url: string) => boolean;
  /** Optional explicit cancel detection (e.g. Paystack cancel_action URL). */
  matchCancel?: (url: string) => boolean;
  /** User closed modal, Paystack success redirect, or cancel redirect. */
      ...
      <SafeAreaView style={styles.safe} edges={["top", "bottom"]}>
        ...
        {authorizationUrl ? (
          <WebView
            ...
            originWhitelist={["https://*", "http://*", "customer://*", "exp://*"]}
