...
export function OnDemandWaitingScreen() {
  ...
  const ow = (key: string, opts?: Record<string, unknown>) =>
    t(`customer.mobile.components.onDemandWaiting.${key}`, opts) as string;
  ...
  const title = uiCopy.waiting_title ?? uiCopy.title ?? ow("defaultTitle");
  ...
  return (
    <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <Text style={{ fontSize: 12, color: Colors.gray[400], marginTop: 16 }}>{ow("timeoutLabel", { seconds: timeoutSec })}</Text>
    </View>
  );
}
