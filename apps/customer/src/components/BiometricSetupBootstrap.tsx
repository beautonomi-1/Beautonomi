...
import { onboardingDoneKey } from "../../app/(app)/onboarding/index";
import {
...
/**
 * One-time opt-in for returning users who completed onboarding before the
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        try {
          const stored = await AsyncStorage.getItem(onboardingDoneKey(userId));
          onboardingComplete = stored === "1";
          ...
          /* skip offer when status unknown */
        }
