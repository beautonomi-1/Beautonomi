...
export default function CustomOfferCheckoutScreen() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      api
        .get<QuotePayload>(`/api/me/custom-offers/${offerId}/quote?${params.toString()}`)
        .then((res) => {
  ...
  const pollForBooking = useCallback(async (): Promise<string | null> => {
    for (let i = 0; i < 30; i += 1) {
      try {
        const state = await api.get<{ booking_id?: string | null }>(
          `/api/me/custom-offers/${offerId}`
        );
        const bookingId =
          (state.data as { booking_id?: string | null } | undefined)?.booking_id ?? null;
  ...
  const handlePay = useCallback(async () => {
    ...
    const opt = providerRequiresDeposit && paymentOption === "deposit" ? "deposit" : "full";
          ...
          >
            ...
            {quote?.pricing ? (
              <>
                ...
                {quote.splits_error ? (
                  <Text style={{ fontSize: 12, color: "#B91C1C", marginTop: 8 }}>
                    {quote.splits_error.message}
                  </Text>
                ) : null}
                <View
