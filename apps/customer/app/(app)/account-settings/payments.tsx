...
function formatCardExpiry(method: any): string | null {
  ...
  if (!method?.expiry_month || !method?.expiry_year) return null;
...
export default function PaymentsScreen() {
    ...
    try {
      ...
      const [methodsRes, giftRes, couponsRes] = await Promise.all([
        ...
        api.get<any>("/api/me/gift-cards").catch((err) => {
          ...
          return {
            data: null,
            error: { message: t("customer.paymentsScreen.loadGiftCardsFailed") },
          };
        }),
      ...
      if (giftRes?.error) {
        setGiftCardsError(
          giftRes.error.message || t("customer.paymentsScreen.loadGiftCardsFailed")
        );
        setGiftCards([]);
      } else {
  ...
  const redeemCoupon = async () => {
    ...
    if (!code) {
      Alert.alert(
        t("customer.paymentsScreen.couponTitle"),
        t("customer.paymentsScreen.enterCouponBody")
      );
      return;
    }
    ...
    try {
      ...
      if (res.error) {
        Alert.alert(
          t("customer.paymentsScreen.couponTitle"),
          res.error.message ?? t("customer.paymentsScreen.couponRedeemFailed")
        );
        return;
      }
      setCouponCode("");
      ...
      Alert.alert(
        t("customer.paymentsScreen.couponTitle"),
        e instanceof Error ? e.message : t("customer.paymentsScreen.couponRedeemFailed")
      );
    } finally {
  ...
  const removeMethod = async (id: string) => {
    Alert.alert(
      ...
      t("customer.paymentsScreen.removeCardConfirm"),
      [
        { text: t("common.cancel"), style: "cancel" },
        {
          text: t("customer.paymentsScreen.remove"),
          style: "destructive",
          onPress: async () => {
            const res = await api.delete<any>(`/api/me/payment-methods/${id}`);
            if (res.error) {
              Alert.alert(
                t("common.error"),
                res.error.message ?? t("customer.paymentsScreen.couldNotRemoveCard")
              );
            } else {
              load();
            }
  ...
  const addCard = async () => {
    ...
    try {
      ...
      if (res.error) {
        Alert.alert(
          t("common.error"),
          res.error.message ?? t("customer.paymentsScreen.couldNotStartVerification")
        );
        return;
      }
      ...
      Alert.alert(
        t("common.error"),
        e instanceof Error ? e.message : t("customer.paymentsScreen.couldNotAddCard")
      );
    } finally {
      setAddingCard(false);
    }
  ...
  const couponCountLabel =
    couponCount === 1
      ? t("customer.paymentsScreen.oneActiveCoupon")
      : t("customer.paymentsScreen.nActiveCoupons", { count: couponCount });
      ...
      <ScreenFrame loading={loading} error={error} onRetry={load}>
        <View>
          <View>
            ...
            >
              <Text style={{ fontWeight: "600", color: Colors.gray[900] }}>
                {t("customer.paymentsScreen.methodsHeading")}
              </Text>
              <TouchableOpacity
                onPress={() =>
                  Alert.alert(t("customer.paymentsScreen.saveCardInfoTitle"), saveCardInfo)
                }
            ...
            {methods.length === 0 ? (
              <Text style={{ color: Colors.gray[500], paddingVertical: 16 }}>
                {t("customer.paymentsScreen.noMethods")}
              </Text>
            ) : (
              methods.map((m) => {
                const expiry = formatCardExpiry(m);
                return (
                  <View
                    key={m.id}
                    style={{
                      ...
                      {expiry ? (
                        <Text
                          style={{
                            marginTop: 2,
                            fontSize: 12,
                            color: m.is_expired ? "#B91C1C" : Colors.gray[500],
                            fontWeight: m.is_expired ? "700" : "400",
                          }}
                        >
                          {m.is_expired ? "Expired" : "Expires"} {expiry}
                        </Text>
                      ) : null}
                    </View>
                    <TouchableOpacity onPress={() => removeMethod(m.id)}>
                      <Text style={{ color: "#B91C1C", fontSize: 14 }}>
                        {t("customer.paymentsScreen.remove")}
                      </Text>
                    </TouchableOpacity>
                  </View>
                );
          ...
          <View style={{ marginTop: 24 }}>
            ...
            >
              <Text style={{ fontWeight: "600", color: Colors.gray[900] }}>
                {t("customer.paymentsScreen.giftCards")}
              </Text>
              <TouchableOpacity onPress={() => router.push("/(app)/gift-card-purchase")}>
                <Text style={{ color: Colors.primary, fontWeight: "500" }}>
                  {t("customer.paymentsScreen.buyGiftCard")}
                </Text>
              </TouchableOpacity>
            </View>
            ...
            {giftCardsError && (
              ...
              >
                <Text style={{ color: "#B91C1C", flex: 1 }} numberOfLines={2}>
                  {giftCardsError}
                </Text>
                <TouchableOpacity
                  onPress={load}
                  ...
                  accessibilityLabel={t("customer.paymentsScreen.retryGiftCardsA11y")}
                >
          ...
          <View style={{ marginTop: 24 }}>
            ...
            >
              <Text style={{ fontWeight: "600", color: Colors.gray[900] }}>
                {t("customer.paymentsScreen.coupons")}
              </Text>
            ...
            <View style={{ backgroundColor: Colors.gray[50], borderRadius: 12, padding: 12 }}>
              <Text style={{ fontSize: 12, color: Colors.gray[600], marginBottom: 8 }}>
                {t("customer.paymentsScreen.couponHelp")}
              </Text>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                ...
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    backgroundColor:
                      couponSubmitting || !couponCode.trim() ? Colors.gray[300] : Colors.primary,
                    minWidth: 96,
                    alignItems: "center",
                  }}
                  accessibilityRole="button"
                ...
                >
                  {couponSubmitting ? (
                    ...
                    <Text style={{ color: Colors.white, fontWeight: "600" }}>
                      {t("customer.paymentsScreen.redeem")}
                    </Text>
                  )}
                </TouchableOpacity>
              </View>
