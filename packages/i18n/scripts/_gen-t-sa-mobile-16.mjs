...
function loadPool() {
  ...
  for (const f of fs.readdirSync(path.join(root, "_maps"))) {
    ...
    if (f === "t-sa-mobile-16.json") continue;
...
const list = JSON.parse(
  fs.readFileSync(path.join(root, "_work/zu-leftover-en-strings.json"), "utf8"),
);
const pool = loadPool();
const map = buildMap(list, pool);
const dest = path.join(root, "_maps", "t-sa-mobile-16.json");
fs.writeFileSync(dest, JSON.stringify(map, null, 2) + "\n");
console.log(`t-sa-mobile-16.json: ${Object.keys(map).length} keys`);
