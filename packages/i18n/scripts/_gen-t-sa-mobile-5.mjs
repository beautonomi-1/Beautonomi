#!/usr/bin/env node
/**
 * Generate _maps/t-sa-mobile-5.json — first 350 strings from mobile-sa-missing-en.json.
 */
...
async function mmTranslate(text, tl) {
  const pair = tl.includes("-") ? tl : `${tl}|en`.split("|").reverse().join("|");
...
const dest = path.join(root, "_maps/t-sa-mobile-5.json");
...
for (const enVal of list) {
  ...
  for (const [loc, tl] of SA) {
    if (typeof row[loc] === "string" && row[loc].trim() && row[loc] !== enVal && !stillMostlyEnglish(enVal, row[loc])) {
      continue;
    }
