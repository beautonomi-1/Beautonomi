...
const TARGETS = ["t-sa-mobile-16.json"];
...
function cellBad(en, val, loc) {
  if (isIdentity(en)) return false;
  if (loc === "zu") return !val || val === en;
...
function smart(en, loc, poolRow) {
  for (const p of valueToPaths.get(en) || []) {
    ...
    if (v && v !== en && (loc === "zu" || !stillMostlyEnglish(en, v))) return v;
  }
  const fromPool = poolRow?.[loc];
  if (fromPool && fromPool !== en && (loc === "zu" || !stillMostlyEnglish(en, fromPool))) return fromPool;
  for (const fn of [translate, translateBestEffort]) {
    const tr = fn(en, loc);
    if (tr && tr !== en && (loc === "zu" || !stillMostlyEnglish(en, tr))) return tr;
    if (tr && tr !== en && en.length < 36) return tr;
  }
  const gloss = applyGlossary(en, loc);
  if (gloss !== en && (loc === "zu" || !stillMostlyEnglish(en, gloss))) return gloss;
  const be = translateBestEffort(en, loc);
  if (be && be !== en && (loc === "zu" || !stillMostlyEnglish(en, be))) return be;
...
for (const mapFile of TARGETS) {
  ...
  for (let pass = 0; pass < 3; pass++) {
    ...
    for (const [enVal, row] of Object.entries(map)) {
      ...
      let changed = false;
      for (const loc of SA) {
        if (!cellBad(enVal, next[loc], loc)) continue;
  ...
  for (const [en, row] of Object.entries(map)) {
    ...
    if (cellBad(en, row.zu, "zu")) zuBad += 1;
    for (const loc of SA) if (cellBad(en, row[loc], loc)) anyBad += 1;
  }
