...
const all = JSON.parse(
  fs.readFileSync(path.join(root, "_work/zu-leftover-not-in-maps.json"), "utf8"),
);
const pool = loadPool();
...
for (const [num, start, end] of batches) {
  const list = all.slice(start, end);
  const map = buildMap(list, pool);
  const dest = path.join(root, "_maps", `t-sa-mobile-${num}.json`);
  fs.writeFileSync(dest, JSON.stringify(map, null, 2) + "\n");
  console.log(`t-sa-mobile-${num}.json: ${Object.keys(map).length} keys (indices ${start}-${end - 1})`);
}
