-- Fix compliance_reset_tenant_transactions (supersedes 508):
-- 1) public.notifications has no tenant_id — 508 used `tenant_id = $1 OR ...` in join-delete.
-- 2) public.booking_holds has no tenant_id — 508 listed it in v_tenant_tables, so phase 1 ran
--    `WHERE tenant_id = $1` first and failed with the same error.
-- 3) Custom service requests / on-demand pre-booking flows: `custom_requests` and `on_demand_requests`
--    are scoped by provider_id (no tenant_id); delete via tenant's providers.
--
-- Notifications: scoped by user_id; delete/count *before* tenant_tables removes `bookings`.
-- booking_holds: delete via provider_id IN (tenant's providers) in join-delete phase.
-- custom_requests: CASCADE removes custom_offers and custom_request_attachments.
