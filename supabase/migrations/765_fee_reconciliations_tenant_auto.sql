-- ============================================================================
-- 765: fee_reconciliations tenant scope + auto daily reconciliations
-- ============================================================================
-- Adds tenant_id, source, recorded_fees for per-tenant daily auto upserts.
-- Extends compliance_reset_tenant_transactions to clear tenant fee reconciliations.

-- ── Schema ──────────────────────────────────────────────────────────────────

ALTER TABLE public.fee_reconciliations
...
ALTER TABLE public.fee_reconciliations
  ADD COLUMN IF NOT EXISTS recorded_fees NUMERIC(10, 2);

ALTER TABLE public.fee_reconciliations
  DROP CONSTRAINT IF EXISTS fee_reconciliations_source_check;

ALTER TABLE public.fee_reconciliations
  ADD CONSTRAINT fee_reconciliations_source_check
  CHECK (source IN ('manual', 'auto_daily'));

-- Legacy manual rows may omit recorded_fees; backfill from actual on read in app.
COMMENT ON COLUMN public.fee_reconciliations.recorded_fees IS
  'Ledger-recorded gateway fees at reconciliation creation (auto_daily uses finance_transactions sum).';

COMMENT ON COLUMN public.fee_reconciliations.source IS
  'manual = admin POST; auto_daily = cron upsert from ledger + fee config.';

-- Allow auto rows without an admin actor.
ALTER TABLE public.fee_reconciliations
  ALTER COLUMN created_by DROP NOT NULL;

ALTER TABLE public.fee_reconciliations
  DROP CONSTRAINT IF EXISTS fee_reconciliations_reconciliation_date_gateway_name_key;

ALTER TABLE public.fee_reconciliations
  DROP CONSTRAINT IF EXISTS fee_reconciliations_date_gateway_tenant_key;

ALTER TABLE public.fee_reconciliations
  ADD CONSTRAINT fee_reconciliations_date_gateway_tenant_key
  UNIQUE NULLS NOT DISTINCT (reconciliation_date, gateway_name, tenant_id);

CREATE INDEX IF NOT EXISTS idx_reconciliations_tenant_date
  ON public.fee_reconciliations(tenant_id, reconciliation_date DESC);

CREATE INDEX IF NOT EXISTS idx_reconciliations_source
  ON public.fee_reconciliations(source, reconciliation_date DESC);


-- Extend compliance reset (764 + fee_reconciliations) --
...
COMMENT ON FUNCTION public.compliance_reset_tenant_transactions(UUID, BOOLEAN, BOOLEAN) IS
  '765: Superadmin clean-slate (764 + fee_reconciliations per tenant). Each op isolated; idempotent; dry-run by default.';
