...
-- 243_freelancer_location_from_user_addresses.sql
-- Fix ensure_freelancer_location to use user_addresses (default or first) instead of non-existent users.address,
-- and set latitude/longitude on the new provider_locations row when available so distance and travel fee work.
...
DECLARE
  ...
  v_lng NUMERIC;
  v_result JSONB;
  -- From user_addresses (default or first)
  v_addr_line1 TEXT;
  v_addr_line2 TEXT;
  v_addr_city TEXT;
  v_addr_state TEXT;
...
BEGIN
  ...
  -- Get user's default or first address from user_addresses (not users.address which does not exist)
  SELECT
    ua.address_line1,
    ua.address_line2,
    ua.city,
  ...
  -- If no location exists, create one from user_addresses when available
  IF NOT v_location_exists AND v_has_address THEN
    ...
    v_result := jsonb_build_object(
      ...
      'has_coordinates', (v_addr_lat IS NOT NULL AND v_addr_lng IS NOT NULL)
    );
  ...
  -- If no service zone exists, create a default one (for at-home services) when location has coordinates
...
COMMENT ON FUNCTION ensure_freelancer_location IS 'Ensures a freelancer has a provider_locations row; creates from user_addresses (default or first) with latitude/longitude when available.';

-- Backfill: run for existing freelancers who have no location but may have a saved user address
DO $$
DECLARE
  v_provider RECORD;
  v_result JSONB;
    ...
    WHERE p.business_type = 'freelancer'
      ...
      AND EXISTS (
        ...
        WHERE ua.user_id = p.user_id
          AND ua.address_line1 IS NOT NULL
          AND ua.city IS NOT NULL
          AND ua.country IS NOT NULL
      )
  LOOP
    BEGIN
      SELECT ensure_freelancer_location(v_provider.id) INTO v_result;
    EXCEPTION
      WHEN OTHERS THEN
        RAISE WARNING 'ensure_freelancer_location failed for provider %: %', v_provider.id, SQLERRM;
    END;
  END LOOP;
END $$;
