-- Persist the originating public booking hold on `bookings` so pre-payment
-- revalidation (`revalidateBookingSlotBeforePayment`) can exclude the customer's
-- own hold from `checkActiveHoldOverlap` (avoids false "slot taken" errors).

ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS hold_id UUID NULL REFERENCES public.booking_holds(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_bookings_hold_id
  ON public.bookings(hold_id)
  WHERE hold_id IS NOT NULL;

COMMENT ON COLUMN public.bookings.hold_id IS 'Public booking hold (`booking_holds.id`) that reserved this slot before insert; used to exclude self from overlap checks at payment time.';

-- Extend create_booking_with_locking to persist hold_id from p_booking_data JSON.
