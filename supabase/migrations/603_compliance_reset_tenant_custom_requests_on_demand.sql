-- Tenant transactional reset: include custom service requests and on-demand accept/decline rows.
-- `custom_requests` / `on_demand_requests` have no tenant_id; scope by provider_id in the tenant.
-- Deleting `custom_requests` CASCADEs `custom_offers` and `custom_request_attachments` (036_custom_orders.sql).
--
-- Apply after 602 if your database already recorded migration 602 (Supabase will not re-run changed 602 SQL).
