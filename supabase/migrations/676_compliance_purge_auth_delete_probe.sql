-- 676: Auth delete probe — surface the exact Postgres FK when diagnose misses blockers.
--
-- Supabase Auth returns only "Database error deleting user". This probe runs
-- DELETE FROM public.users then auth.users inside a nested subtransaction (rolled
-- back via BEGIN … EXCEPTION) so compliance_clear_user_references fails with the
-- real constraint detail.

CREATE OR REPLACE FUNCTION public.compliance_probe_auth_user_delete(p_user_id UUID)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- PL/pgSQL functions cannot use SAVEPOINT / ROLLBACK TO SAVEPOINT. A nested
  -- BEGIN … EXCEPTION sub-block rolls back DELETEs on any exit (success or error).
  BEGIN
    DELETE FROM public.users WHERE id = p_user_id;
    DELETE FROM auth.users WHERE id = p_user_id;
    RAISE EXCEPTION 'compliance_probe_success'
      USING ERRCODE = 'P0002';
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      IF _detail IS NOT NULL AND btrim(_detail) <> '' THEN
        RETURN _detail;
      END IF;
      RETURN _msg;
  END;
...
COMMENT ON FUNCTION public.compliance_probe_auth_user_delete(UUID) IS
  'Rolled-back DELETE probe for public.users + auth.users (nested subtransaction); returns ''ready'' or Postgres FK detail.';

REVOKE ALL ON FUNCTION public.compliance_probe_auth_user_delete(UUID) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.compliance_probe_auth_user_delete(UUID) FROM anon;
REVOKE ALL ON FUNCTION public.compliance_probe_auth_user_delete(UUID) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.compliance_probe_auth_user_delete(UUID) TO service_role;

-- Replace diagnose-only pass 6 with the probe (strict superset for failure messages).
...
BEGIN
  ...
  _probe := public.compliance_probe_auth_user_delete(p_user_id);
  IF _probe IS DISTINCT FROM 'ready' THEN
    RAISE EXCEPTION 'Auth delete would fail: %', _probe USING ERRCODE = '23503';
  END IF;
END;
$$;

COMMENT ON FUNCTION public.compliance_clear_user_references(UUID) IS
  'Superadmin compliance: clear FK blockers before auth.admin.deleteUser; ends with rolled-back auth delete probe.';
