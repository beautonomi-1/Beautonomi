-- §provider-launch (2026-06): harden compliance_clear_user_references for
-- provider deletion.
--
-- Deleting a provider OWNER cascades public.users -> providers -> offerings.
-- Beyond booking_services.offering_id (handled in migration 631 by deleting the
-- provider's bookings first), other transactional tables RESTRICT / NO ACTION
-- against providers(id) or offerings(id) — e.g. sales, sale_items, product_orders,
-- payout/invoice rows. When any of those rows exist, the auth delete fails with
-- the opaque "Database error deleting user", which surfaced to providers as the
-- generic "Failed to delete account".
--
-- Fix: before the existing users/auth FK cleanup, dynamically clear every
-- public child row that RESTRICT/NO ACTION references the target user's
-- providers/offerings. Offerings-referencing children are cleared before
-- providers-referencing children so nested chains resolve. The precise
-- FK-blocker RAISE is preserved so any remaining blocker is reported by name.
...
BEGIN
  ...
  -- Provider-owner purge: remove transactional data that blocks provider → offerings CASCADE.
  ...
  -- §provider-launch (2026-06): clear provider-owned RESTRICT / NO ACTION chains
  -- against providers(id) and offerings(id). offerings-referencing children are
  -- cleared first (see ORDER BY below) so chains such as sale_items.offering_id
  -- resolve before their parent-side rows are removed by the CASCADE from
  -- public.users.
  FOR _fk IN
    SELECT
    ...
    WHERE con.contype = 'f'
      ...
      -- 'a' = NO ACTION, 'r' = RESTRICT. CASCADE / SET NULL are handled by the
      -- database when public.users is deleted.
      AND con.confdeltype IN ('a', 'r')
      -- Never self-delete providers/offerings here; the users CASCADE removes
      -- them once their blocking children are gone.
      AND child.relname NOT IN ('providers', 'offerings')
    -- 'offerings' < 'providers' alphabetically, so ASC clears offerings-children
    -- first. This matters when a providers-child row is itself referenced by an
    -- offerings-child (delete the deeper dependent before its parent-side row).
...
COMMENT ON FUNCTION public.compliance_clear_user_references(UUID) IS
  'Superadmin compliance: clear FK blockers before auth.admin.deleteUser. Deletes owned-provider bookings, then provider/offerings RESTRICT chains (sales, product_orders, payouts, etc.), then dynamic public FK cleanup for users/auth.users.';
