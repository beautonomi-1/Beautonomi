-- Migration 604: Align non-Paystack booking_payments ledger commission base with Paystack webhook.
-- The Paystack path (charge-success) excludes tip_amount from the commission base because tips
-- are recorded as separate `tip` finance_transactions. The trigger path previously omitted tip,
-- inflating provider_earnings when tips > 0 and double-counting with the tip line.
...
BEGIN
    ...
    IF v_is_online_booking THEN
        v_net_ratio := GREATEST(
            0,
            (v_total_amount - v_booking.platform_fee_amount - v_booking.tax_amount - v_booking.travel_fee - v_booking.tip_amount)
        ) / v_total_amount;
    ...
    ELSE
        v_net_ratio := GREATEST(
            0,
            (v_total_amount - v_booking.tax_amount - v_booking.travel_fee - v_booking.tip_amount)
        ) / v_total_amount;
        v_commission_base := ROUND(v_payment_amount * v_net_ratio, 2);
...
COMMENT ON FUNCTION public.create_finance_ledger_from_payment() IS
  'Creates finance ledger rows for completed booking payments (non-Paystack). Excludes tip from commission base (matches Paystack webhook). Uses COALESCE(NULLIF(...)) for platform_fee_amount vs legacy service_fee_amount.';
