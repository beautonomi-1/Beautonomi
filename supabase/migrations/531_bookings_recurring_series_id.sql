-- Link bookings generated from (or tied to) a customer recurring series so clients can
-- list "all visits from this repeat" in one query (`WHERE recurring_series_id = :series_id`).

ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS recurring_series_id UUID REFERENCES public.recurring_appointments(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_bookings_recurring_series_id
  ON public.bookings(recurring_series_id)
  WHERE recurring_series_id IS NOT NULL;

COMMENT ON COLUMN public.bookings.recurring_series_id IS 'Customer recurring series (recurring_appointments.id) that produced or owns this visit; set by cron / checkout recurring flows.';

-- Extend create_booking_with_locking to persist recurring_series_id from p_booking_data JSON.
