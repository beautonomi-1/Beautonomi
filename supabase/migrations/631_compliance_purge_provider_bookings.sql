-- Compliance user / provider-org purge: delete owned-provider bookings before auth delete.
--
-- When auth.users is removed, public.users CASCADE deletes providers, which CASCADE
-- deletes offerings. booking_services.offering_id → offerings(id) is RESTRICT
-- (migration 005) so Postgres can fail with:
--   "update or delete on table offerings violates foreign key constraint
--    booking_services_offering_id_fkey"
-- because sibling CASCADE paths (bookings vs offerings) are not ordered.
--
-- Fix: in compliance_clear_user_references, delete all bookings for providers
-- owned by the target user first (booking_services/booking_addons CASCADE from
-- booking_id). Offerings catalogue uses soft-delete in normal ops; hard delete
-- only happens on compliance purge.
...
COMMENT ON FUNCTION public.compliance_clear_user_references(UUID) IS
  'Superadmin compliance: clear FK blockers before auth.admin.deleteUser. Deletes owned-provider bookings first (offerings RESTRICT vs booking_services), then dynamic public FK cleanup for users/auth.users.';
