-- Residual earnings allocation for subsequent booking_payments when booking-level
-- ledger legs (tip/travel/platform_fee/etc.) were already posted by an earlier payment.
-- Fixes diluted provider_earnings on post-edit cash/card top-ups (e.g. EDFD02D5).
-- Also updates the source_payment_id single-writer index comment: webhooks may now
-- set source_payment_id on gateway-generated finance_transactions rows.
...
BEGIN
    ...
    -- Check before earnings math: booking-level legs already posted by a prior payment?
    ...
    IF v_has_existing_booking_items THEN
        -- Residual allocation: booking-level legs already posted in full,
        -- so this payment funds only the still-unposted service revenue.
...
COMMENT ON INDEX public.ux_finance_transactions_source_payment_per_type IS
  'Single-writer contract per (source_payment_id, transaction_type): the create_finance_ledger_from_payment '
  'trigger writes rows for non-gateway booking_payments; Paystack/Stripe/Flutterwave webhook handlers '
  'write rows with source_payment_id set to the booking_payments.id created in the same flow. '
  'Wallet/gift-card-only fallback paths (no booking_payments row) may also write app-side.';
