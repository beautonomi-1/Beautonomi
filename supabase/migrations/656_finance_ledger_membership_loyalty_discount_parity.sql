-- 656: non-Paystack ledger trigger — membership & loyalty discount parity
--
-- Extends create_finance_ledger_from_payment() (last set in 605) so wallet/gift/
-- cash settlements also post membership_discount and loyalty_redemption contra
-- rows, mirroring the promotion_discount row already emitted here and matching the
-- Paystack webhook (charge-success) and finalize-custom-offer paths. This closes
-- the GMV-vs-net asymmetry for in-person/no-gateway bookings. Both new rows are
-- net-negative contra-revenue (not added to total_amount) and are gated by the
-- existing per-booking item guard so retries do not duplicate them.
