-- Atomic settlement for provider-collected additional charges.
-- Keeps payment audit, charge status, booking totals, and non-payout ledger
-- rows consistent across retries and partial failures.
...
DECLARE
  ...
  v_existing_charge_payment record;
...
BEGIN
  ...
  -- If the charge is already paid and already has an audit transaction, do not
  -- create a second audit row just because a retry supplied a different
  -- reference. Continue to the ledger recovery section below.
  IF v_charge_was_paid THEN
    SELECT id, reference
      INTO v_existing_charge_payment
  ...
  IF v_existing_charge_payment.id IS NULL THEN
  ...
  -- A duplicate reference is only idempotent if it belongs to this exact charge.
  -- Otherwise a reused terminal/reference could incorrectly settle another charge.
  IF v_payment_tx_id IS NULL AND v_existing_charge_payment.id IS NULL THEN
  ...
  IF NOT v_charge_was_paid THEN
    ...
    SET total_amount = COALESCE(total_amount, 0) + COALESCE(v_charge.amount, 0),
        total_paid = COALESCE(total_paid, 0) + COALESCE(v_charge.amount, 0),
        updated_at = now()
    WHERE id = p_booking_id;
