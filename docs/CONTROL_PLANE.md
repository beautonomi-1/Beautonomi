...
## Existing config endpoints and tables (repo scan)

The following records existing config endpoints and tables used by the Control Plane. **Do not remove or replace these; orchestrate via the config service.**
...
- **feature_flags**: `id`, `feature_key` (UNIQUE), `feature_name`, `description`, `enabled`, `category`, `metadata` (JSONB), `created_at`, `updated_at`, `created_by`, `updated_by`. Extend via migration for rollout_percent, platforms_allowed, etc.
