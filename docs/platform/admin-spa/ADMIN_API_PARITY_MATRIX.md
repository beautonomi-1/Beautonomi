...
|-------|---------|-----------------------------------|
| **Inventory** | **355** admin `route.ts` handlers; full list in `docs/admin-api-route-taxonomy.csv` | Regenerate CSV when adding routes; CI blocks orphan files. |
...
| 10 | `/admin/providers/[id]` | W1 | providers_operations | N | `GET/PATCH /api/admin/providers/:id`, payout accounts; `GET/PATCH/POST /api/admin/providers/:id/yoco` | **SPA:** [`ProviderDetailPage`](../../apps/admin-web/src/routes/providers/ProviderDetailPage.tsx) — Yoco credential panel (`credential_mode`, OAuth disconnect). AuthZ `requireRoleInApi` + `resolveAdminApiTenantId`. |
...
| 20 | `/admin/payouts` | W2 | finance | Y | `GET/POST/PATCH /api/admin/payouts`; `POST .../:id/finalize-transfer` (Paystack OTP) | **SPA:** [`PayoutsPage`](../../apps/admin-web/src/routes/finance/PayoutsPage.tsx) — finalize transfer modal. AuthZ `ADMIN_SECTION_FINANCE`, `resolveAdminApiTenantId`. |
