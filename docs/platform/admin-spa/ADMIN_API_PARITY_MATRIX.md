...
|-------|---------|-----------------------------------|
| **Inventory** | **408** admin `route.ts` handlers; full list in `docs/admin-api-route-taxonomy.csv` | Regenerate CSV when adding routes; CI blocks orphan files. |
...
| 10 | `/admin/providers/[id]` | W1 | providers_operations | N | `GET/PATCH /api/admin/providers/:id`, payout accounts; `GET/PATCH/POST /api/admin/providers/:id/yoco`; `GET/POST /api/admin/providers/:id/marketing-credits`; `GET/PATCH .../payment-terminal-profile`; `GET/PATCH .../terminal-integrations`; `GET/POST .../terminal-notes` | **SPA:** [`ProviderDetailPage`](../../apps/admin-web/src/routes/providers/ProviderDetailPage.tsx) — Yoco credential panel (`credential_mode`, OAuth disconnect). AuthZ `requireRoleInApi` + `resolveAdminApiTenantId`. Marketing-credits panel ([`ProviderMarketingCreditsPanel`](../../apps/admin-web/src/components/marketing/ProviderMarketingCreditsPanel.tsx)) shows balance + ledger and posts manual grant/debit adjustments — `ADMIN_SECTION_MARKETING_COMMS`, audit-logged (`admin.provider.marketing_credits.adjust`). Commercial tab ([`ProviderCommercialTab`](../../apps/admin-web/src/routes/providers/tabs/ProviderCommercialTab.tsx)) — payment-terminal profile, vendor integrations, admin notes (`ADMIN_SECTION_COMMERCIAL`). |
