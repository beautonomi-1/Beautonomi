...
| `commercial` | Commercial Operations | `admin_finance`, `admin_operations` | `ADMIN_SECTION_COMMERCIAL` |
...
## 4. Seeded page → primary API inventory
...
| 6a | `/admin/knowledge-base` | W0 | overview | Y | `GET /api/admin/learning/browse`, `GET /api/admin/learning/search`, `GET /api/admin/learning/articles/:slug` | **SPA:** [`KnowledgeBasePage`](../../apps/admin-web/src/routes/learning/KnowledgeBasePage.tsx) + [`KnowledgeBaseArticlePage`](../../apps/admin-web/src/routes/learning/KnowledgeBaseArticlePage.tsx) — internal staff reader over Learning Center content (incl. `is_internal` runbooks); interactive mockups replaced with placeholder + live `/learn` link. AuthZ `ADMIN_SECTION_OVERVIEW` (any admin role), tenant via `resolveAdminApiTenantId`. Search also powers the support desk article picker ([`LearningArticlePicker`](../../apps/admin-web/src/components/learning/LearningArticlePicker.tsx)). |
| 7 | `/admin/support-tickets` | W1 | support | Y | `GET /api/admin/support-tickets`, `POST .../support-tickets/bulk` | **SPA (pattern wave):** list + filters; KPI strip; bulk assign/status; `status`/`priority`/`category`/`assign`/`page`/`q` in URL; row action → legacy detail (row 8). |
| 8 | `/admin/support-tickets/[id]` | W1 | support | N | `GET/PATCH .../support-tickets/:id`, `POST .../messages`, `.../notes`, `.../seen` (stamps `last_staff_view_at`), `GET /api/admin/support-ticket-assignees` | |
| 9 | `/admin/providers` | W1 | providers_operations | Y | `GET /api/admin/providers`, `PATCH .../status`, `.../verify`, `POST .../bulk` | |
| 10 | `/admin/providers/[id]` | W1 | providers_operations | N | `GET/PATCH /api/admin/providers/:id`, payout accounts; `GET/PATCH/POST /api/admin/providers/:id/yoco`; `GET/PATCH /api/admin/providers/:id/paycloud`; `GET/PATCH /api/admin/providers/:id/payee-entity`; `GET/POST /api/admin/providers/:id/marketing-credits`; `GET/PATCH .../payment-terminal-profile`; `GET/PATCH .../terminal-integrations`; `GET/POST .../terminal-notes` | **SPA:** [`ProviderDetailPage`](../../apps/admin-web/src/routes/providers/ProviderDetailPage.tsx) — Yoco credential panel (`credential_mode`, OAuth disconnect). AuthZ `requireRoleInApi` + `resolveAdminApiTenantId`. Marketing-credits panel ([`ProviderMarketingCreditsPanel`](../../apps/admin-web/src/components/marketing/ProviderMarketingCreditsPanel.tsx)) shows balance + ledger and posts manual grant/debit adjustments — `ADMIN_SECTION_MARKETING_COMMS`, audit-logged (`admin.provider.marketing_credits.adjust`). Commercial tab ([`ProviderCommercialTab`](../../apps/admin-web/src/routes/providers/tabs/ProviderCommercialTab.tsx)) — PayCloud card-machine readiness/terminals/payments (`.../paycloud`, `.../paycloud-operations/*`), payment-terminal profile, vendor integrations, admin notes (`ADMIN_SECTION_COMMERCIAL`). Overview tab ([`ProviderPayeeEntityPanel`](../../apps/admin-web/src/routes/providers/components/ProviderPayeeEntityPanel.tsx)) — KYB payee entity (`.../payee-entity`). |
...
| 38a | `/admin/identity-trust/sessions` | W3 | users_trust | Y (superadmin only) | `GET /api/admin/identity-verification/sessions`; `GET .../sessions/:id`; `POST .../sessions/:id/override`, `.../resend`, `.../resolve-flag`, `.../reprocess-webhook`; `GET /api/admin/identity-verification/metrics` | **SPA:** [`VerificationSessionsPage`](../../apps/admin-web/src/routes/identity-trust/VerificationSessionsPage.tsx). Superadmin-only Didit session ops console. AuthZ `requireRoleInApi(["superadmin"])`. |
...
| 54 | `/admin/broadcast` | W4 | marketing_comms | Y | `GET /api/admin/broadcast/audience-preview`; `GET /api/admin/broadcast/[id]`; `GET/POST/PATCH /api/admin/broadcast` (+ related push/sms/email/history) | |
| 55 | `/admin/automations` | W4 | marketing_comms | Y | `GET/POST /api/admin/automations` | |
| 56 | `/admin/notification-templates` | W4 | marketing_comms | Y | Template CRUD under `/api/admin/notification-templates`; `POST .../:id/whatsapp/push`; `POST .../whatsapp/validate-readiness` | **SPA:** [`NotificationTemplatesListPage`](../../apps/admin-web/src/routes/marketing/NotificationTemplatesListPage.tsx) — WhatsApp body editor pushes content to Twilio (`/:id/whatsapp/push`) with live approval-readiness check (`/whatsapp/validate-readiness`). AuthZ `ADMIN_SECTION_MARKETING_COMMS`. |
| 56a | `/admin/whatsapp-content-templates` | W4 | marketing_comms | Y | `GET /api/admin/whatsapp/content-templates`; `POST .../content-templates/push-all`; `POST .../content-templates/sync-all`; `GET /api/admin/whatsapp/delivery-log` | **SPA:** [`WhatsAppContentTemplatesPage`](../../apps/admin-web/src/routes/marketing/WhatsAppContentTemplatesPage.tsx) — WhatsApp-capable template list with sync/drift status, bulk push/sync to Twilio Content API, and recent delivery log. AuthZ `ADMIN_SECTION_MARKETING_COMMS`, tenant via `resolveAdminApiTenantId`. |
| 56b | `/admin/marketing-pricebook` | W4 | marketing_comms | Y | `GET/PATCH /api/admin/marketing/pricebook` | **SPA:** [`MarketingPricebookPage`](../../apps/admin-web/src/routes/marketing/MarketingPricebookPage.tsx) — per-channel/category marketing-credit unit costs (`marketing_channel_pricebook` upsert). AuthZ `ADMIN_SECTION_MARKETING_COMMS`. |
...
| 61d | `/admin/integrations/paycloud` | W4 | integrations_dev | Y (superadmin only) | `GET/PATCH /api/admin/integrations/paycloud`; `POST .../paycloud/test-credentials`; `GET .../paycloud/sandbox-fixtures`; `GET/POST/PATCH /api/admin/paycloud-operations/merchants`; `POST /api/admin/paycloud-operations/reconcile` | **SPA:** [`PayCloudIntegrationPage`](../../apps/admin-web/src/routes/integrations/PayCloudIntegrationPage.tsx). Tenant PayCloud app credentials (live/sandbox), published sandbox fixtures, feature-flag snapshot, merchant registry, credential test + reconciliation trigger. AuthZ `ADMIN_SECTION_INTEGRATIONS_DEV`. |
| 61e | `/admin/integrations/paycloud-operations` | W4 | integrations_dev | Y (superadmin only) | `GET /api/admin/paycloud-operations/payments`; `GET .../payments/:id`; `POST .../payments/:id/force-settle`; `GET/PATCH/POST /api/admin/paycloud-operations/terminals`; `GET/POST/PATCH /api/admin/paycloud-operations/merchants`; `POST /api/admin/paycloud-operations/reconcile` | **SPA:** [`PayCloudOperationsPage`](../../apps/admin-web/src/routes/integrations/PayCloudOperationsPage.tsx) + [`PaycloudPaymentDetailModal`](../../apps/admin-web/src/routes/integrations/PaycloudPaymentDetailModal.tsx). Payment exception queue, payment diagnostics detail, terminal fleet management, merchant registry, bulk reconcile. Also consumed from [`ProviderCommercialTab`](../../apps/admin-web/src/routes/providers/tabs/ProviderCommercialTab.tsx) for per-provider terminal assign/force-settle. AuthZ `ADMIN_SECTION_INTEGRATIONS_DEV`, tenant via `resolveAdminApiTenantId`. |
| 61f | `/admin/integrations/calls` | W4 | integrations_dev | Y (superadmin only) | `GET/PUT /api/admin/integrations/calls` | **SPA:** [`CallsIntegrationPage`](../../apps/admin-web/src/routes/integrations/CallsIntegrationPage.tsx). Twilio Voice + Salestrail webhook credentials, feature toggles, TwiML/webhook URLs. AuthZ `ADMIN_SECTION_INTEGRATIONS_DEV`. Dialer availability: `GET /api/admin/provider-ops/voice/config` + `POST .../voice/token` from [`LeadVoiceDialer`](../../apps/admin-web/src/components/provider-ops/LeadVoiceDialer.tsx). |
| 61g | `/admin/integrations/shipping` | W4 | integrations_dev | Y (superadmin only) | `GET/PATCH /api/admin/integrations/shipping`; `POST .../shipping/probe` | **SPA:** [`ShippingIntegrationPage`](../../apps/admin-web/src/routes/integrations/ShippingIntegrationPage.tsx). Platform `platform_secrets` courier credentials (Courier Guy, Bob Go, Aramex) + live rate-quote probe (no waybill). AuthZ `requireRoleInApi(["superadmin"])`. |
...
| 65 | `/admin/service-zones` | W4 | operations | Y (`admin_operations` + superadmin; APIs: `ADMIN_SECTION_OPERATIONS`) | `GET/POST/PATCH/DELETE .../service-zones`, map layers, include/exclude, publish, `POST .../areas/resolve-point`, areas/search | |
...
| 100 | `/admin/provider-ops/leads` | W5 | provider_ops | Y | `GET /api/admin/provider-ops/leads`, `GET .../assignable-users`, `POST .../leads/:id/restore`, `GET .../voice/config`, `POST .../voice/token` | Lead inbox: list, search, filter by stage/source/country; inline assignee picker; soft-delete restore; in-browser Twilio dialer ([`LeadVoiceDialer`](../../apps/admin-web/src/components/provider-ops/LeadVoiceDialer.tsx)). |
| 101 | `/admin/provider-ops/leads/new` | W5 | provider_ops | N | `POST /api/admin/provider-ops/leads`, `POST /api/mapbox/geocode`, `GET .../categories`, `GET .../referrers/search` | Lead creation form: geocoding, category multi-select, phone E.164, duplicate check; referrer typeahead ([`ReferrerPicker`](../../apps/admin-web/src/components/provider-ops/ReferrerPicker.tsx)). |
| 102 | `/admin/provider-ops/leads/[id]` | W5 | provider_ops | N | `GET/PATCH .../leads/:id`, `PATCH .../stage`, `PATCH .../assign`, `GET/POST .../activities`, `GET/POST/PATCH/DELETE .../tasks`, `GET .../communications`, `GET .../assignable-users`, `POST .../restore`, `POST .../lookup`, `GET .../referrers/search`, `GET .../voice/config`, `POST .../voice/token` | Lead detail: timeline, stage change, assignment, follow-up tasks, communications log, notes, create-account action; soft-delete restore; Twilio phone lookup (API; SPA displays cached `phone_lookup_status`); referrer picker; in-browser dialer. |
...
| 105 | `/admin/provider-ops/reports` | W5 | provider_ops | Y | `GET .../reports/funnel`, `GET .../reports/step-dropoff`, `GET .../reports/lead-assignees`, `GET .../reports/previous-software` | Funnel metrics, step drop-off analysis, lead pipeline, admin productivity, lead-assignee report, previous-software migration report |
...
| 109 | `/admin/commercial/terminal-insights` | W5 | commercial | Y | `GET /api/admin/commercial/terminal-insights`, `GET .../terminal-insights/recipients`, `GET/POST .../terminal-upsell-leads`, `PATCH .../terminal-upsell-leads/:id`, `GET .../terminal-upsell-leads/:id/activities`, `GET .../commercial/assignable-users` | **SPA:** [`TerminalInsightsPage`](../../apps/admin-web/src/routes/commercial/TerminalInsightsPage.tsx) — upsell segment KPIs, pipeline side panel, CSV export, **Broadcast to segment** → [`BroadcastComposePage`](../../apps/admin-web/src/routes/marketing/BroadcastComposePage.tsx) via `GET .../recipients`. AuthZ `ADMIN_SECTION_COMMERCIAL`, tenant via `resolveAdminApiTenantId`. |
| 110 | `/admin/commercial/terminal-products` | W5 | commercial | Y | `GET/POST /api/admin/commercial/terminal-products`; `PATCH .../terminal-products/:id` | **SPA:** [`TerminalProductsPage`](../../apps/admin-web/src/routes/commercial/TerminalProductsPage.tsx). AuthZ `ADMIN_SECTION_COMMERCIAL`. |
| 111 | `/admin/commercial/terminal-orders` | W5 | commercial | Y | `GET /api/admin/commercial/terminal-orders`; `PATCH .../terminal-orders/:id` | **SPA:** [`TerminalOrdersPage`](../../apps/admin-web/src/routes/commercial/TerminalOrdersPage.tsx). AuthZ `ADMIN_SECTION_COMMERCIAL`. |
| 112 | `/admin/commercial/terminal-campaigns` | W5 | commercial | Y | `GET /api/admin/commercial/terminal-campaigns` | **SPA:** [`TerminalCampaignsPage`](../../apps/admin-web/src/routes/commercial/TerminalCampaignsPage.tsx). AuthZ `ADMIN_SECTION_COMMERCIAL`. |
| 113 | `/admin/commercial/terminal-reporting` | W5 | commercial | Y | `GET /api/admin/commercial/terminal-reporting` | **SPA:** [`TerminalReportingPage`](../../apps/admin-web/src/routes/commercial/TerminalReportingPage.tsx). AuthZ `ADMIN_SECTION_COMMERCIAL`. |
| 114 | `/admin/commercial/terminal-vendors` | W5 | commercial | Y | `GET /api/admin/commercial/terminal-vendors`; `GET/PATCH .../terminal-vendors/:vendor` | **SPA:** [`TerminalVendorsPage`](../../apps/admin-web/src/routes/commercial/TerminalVendorsPage.tsx). AuthZ `ADMIN_SECTION_COMMERCIAL`. |
| 115 | `/admin/commercial/terminal-collection-locations` | W5 | commercial | Y | `GET/POST /api/admin/commercial/terminal-collection-locations`; `PATCH/DELETE .../terminal-collection-locations/:id` | **SPA:** [`TerminalCollectionLocationsPage`](../../apps/admin-web/src/routes/commercial/TerminalCollectionLocationsPage.tsx). AuthZ `ADMIN_SECTION_COMMERCIAL`. |
| 116 | `/admin/commercial/terminal-onboarding` | W5 | commercial | Y | `GET/POST /api/admin/terminal-merchant-applications` | **SPA:** [`TerminalOnboardingPage`](../../apps/admin-web/src/routes/commercial/TerminalOnboardingPage.tsx) — list + create-on-behalf. AuthZ `requireTerminalMerchantAdmin` → `ADMIN_SECTION_COMMERCIAL`, tenant via `resolveAdminApiTenantId`. |
| 116a | `/admin/commercial/terminal-onboarding/:id` | W5 | commercial | Y | `GET/PATCH /api/admin/terminal-merchant-applications/[id]`; `POST .../[id]/status`; `POST .../[id]/approve`; `GET .../[id]/export`; `GET/PATCH/POST .../[id]/documents/[docId]` | **SPA:** [`TerminalOnboardingDetailPage`](../../apps/admin-web/src/routes/commercial/TerminalOnboardingDetailPage.tsx) — review, status/term-sheet transitions, document review/upload, PayCloud merchant approve, JSON export. AuthZ `ADMIN_SECTION_COMMERCIAL`. |
...
| 2026-06-21 | **Taxonomy / CI:** Added **11** rows for the Learning Center refresh + WhatsApp marketing-credits work — Knowledge Base reader (`GET /api/admin/learning/browse`, `.../search`, `.../articles/[slug]`; `ADMIN_SECTION_OVERVIEW`), WhatsApp content templates (`GET /api/admin/whatsapp/content-templates`, `.../content-templates/push-all`, `.../content-templates/sync-all`, `GET /api/admin/whatsapp/delivery-log`), notification-template WhatsApp ops (`POST /api/admin/notification-templates/[id]/whatsapp/push`, `.../whatsapp/validate-readiness`), marketing pricebook (`GET/PATCH /api/admin/marketing/pricebook`), and provider marketing credits (`GET/POST /api/admin/providers/[id]/marketing-credits`) — all `ADMIN_SECTION_MARKETING_COMMS` except Knowledge Base. SPA: [`KnowledgeBasePage`](../../apps/admin-web/src/routes/learning/KnowledgeBasePage.tsx), [`KnowledgeBaseArticlePage`](../../apps/admin-web/src/routes/learning/KnowledgeBaseArticlePage.tsx), [`WhatsAppContentTemplatesPage`](../../apps/admin-web/src/routes/marketing/WhatsAppContentTemplatesPage.tsx), [`MarketingPricebookPage`](../../apps/admin-web/src/routes/marketing/MarketingPricebookPage.tsx), [`ProviderMarketingCreditsPanel`](../../apps/admin-web/src/components/marketing/ProviderMarketingCreditsPanel.tsx). §1.1 inventory **384**; §4 rows **6a**, **10**, **56**, **56a**, **56b**. (Taxonomy rows added additively to preserve curated `class`/`note` columns rather than full regen.) |
| 2026-07-04 | **Taxonomy / CI:** Regenerated `docs/admin-api-route-taxonomy.csv` (**408** rows) for commercial terminal ops (`/api/admin/commercial/terminal-*`, `.../terminal-vendors/[vendor]`), Didit identity verification (`/api/admin/control-plane/integrations/didit`, `.../didit/test`, `/api/admin/identity-verification/*`), provider terminal profile/integrations/notes, and subscription receipt PDF (`/api/admin/provider-subscriptions/receipts/[financeTxId]/pdf`). Removed Sumsub control-plane routes. SPA: commercial terminal pages, [`CpIntegrationDiditPage`](../../apps/admin-web/src/routes/control-plane/CpIntegrationDiditPage.tsx), [`VerificationSessionsPage`](../../apps/admin-web/src/routes/identity-trust/VerificationSessionsPage.tsx), [`ProviderCommercialTab`](../../apps/admin-web/src/routes/providers/tabs/ProviderCommercialTab.tsx), finance receipt download. §4 rows **10**, **19**, **38a**, **80**, **109–114**. |
| 2026-07-10 | **Taxonomy / CI:** Regenerated `docs/admin-api-route-taxonomy.csv` (**422** rows) for PayCloud card-machine integration (`/api/admin/integrations/paycloud`, `.../test-credentials`, `/api/admin/paycloud-operations/*`), provider PayCloud admin (`/api/admin/providers/[id]/paycloud`), and provider KYB payee entity (`/api/admin/providers/[id]/payee-entity`). SPA: [`PayCloudIntegrationPage`](../../apps/admin-web/src/routes/integrations/PayCloudIntegrationPage.tsx), [`PayCloudOperationsPage`](../../apps/admin-web/src/routes/integrations/PayCloudOperationsPage.tsx), [`ProviderCommercialTab`](../../apps/admin-web/src/routes/providers/tabs/ProviderCommercialTab.tsx), [`ProviderPayeeEntityPanel`](../../apps/admin-web/src/routes/providers/components/ProviderPayeeEntityPanel.tsx). §4 rows **10**, **61d**, **61e**. |
| 2026-07-14 | **Taxonomy / CI:** Regenerated `docs/admin-api-route-taxonomy.csv` (**437** rows) for Twilio Voice / Salestrail calls integration (`GET/PUT /api/admin/integrations/calls`), provider-ops in-browser dialer (`GET .../voice/config`, `POST .../voice/token`), lead soft-delete restore (`POST .../leads/[id]/restore`), Twilio phone lookup (`POST .../leads/[id]/lookup`), and referrer typeahead (`GET .../referrers/search`). SPA: [`CallsIntegrationPage`](../../apps/admin-web/src/routes/integrations/CallsIntegrationPage.tsx), [`LeadVoiceDialer`](../../apps/admin-web/src/components/provider-ops/LeadVoiceDialer.tsx), [`ReferrerPicker`](../../apps/admin-web/src/components/provider-ops/ReferrerPicker.tsx), [`ProviderOpsLeadsPage`](../../apps/admin-web/src/routes/provider-ops/ProviderOpsLeadsPage.tsx), [`ProviderOpsLeadDetailPage`](../../apps/admin-web/src/routes/provider-ops/ProviderOpsLeadDetailPage.tsx), [`ProviderOpsLeadNewPage`](../../apps/admin-web/src/routes/provider-ops/ProviderOpsLeadNewPage.tsx). §4 rows **61f**, **100–102**. |
...
| 2026-06-21 | Taxonomy: `+11` routes for Learning Center / WhatsApp marketing credits (learning `browse`/`search`/`articles/[slug]`; whatsapp `content-templates` + `push-all`/`sync-all` + `delivery-log`; notification-template `whatsapp/push` + `whatsapp/validate-readiness`; `marketing/pricebook`; `providers/[id]/marketing-credits`); §4 rows **6a**, **10**, **56**, **56a**, **56b**; SPA Knowledge Base reader, WhatsApp content templates, marketing pricebook, provider marketing-credits panel. §1.1 inventory **384**. |
| 2026-07-04 | Taxonomy regen **408** rows: commercial terminal APIs, Didit identity verification (replaces Sumsub), provider terminal profile/integrations/notes, subscription receipt PDF; §4 rows **10**, **19**, **38a**, **80**, **109–114**; section `commercial` (`ADMIN_SECTION_COMMERCIAL`). |
| 2026-07-10 | Taxonomy regen **422** rows: PayCloud integration + operations (`/api/admin/integrations/paycloud`, `/api/admin/paycloud-operations/*`), provider PayCloud admin (`/api/admin/providers/[id]/paycloud`), provider KYB payee entity (`/api/admin/providers/[id]/payee-entity`); §4 rows **10**, **61d**, **61e**; SPA PayCloud integration/operations pages + provider commercial/overview panels. |
| 2026-07-11 | Taxonomy: `+9` routes (commercial `assignable-users`, `terminal-insights/recipients`, `terminal-upsell-leads` CRUD + activities; provider-ops lead `tasks`, `communications`; support-tickets `bulk`); §4 rows **7**, **102**, **109**; terminal upsell pipeline + broadcast segment handoff. §1.1 inventory **431**. |
| 2026-07-14 | Taxonomy regen **437** rows: calls integration (`/api/admin/integrations/calls`), provider-ops voice dialer (`.../voice/config`, `.../voice/token`), lead restore/lookup, referrer search; §4 rows **61f**, **100–102**; SPA calls settings page + provider-ops dialer/referrer/restore. |
