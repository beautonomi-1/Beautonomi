...
| 61d | `/admin/integrations/paycloud` | W4 | integrations_dev | Y (superadmin only) | `GET/PATCH /api/admin/integrations/paycloud`; `POST .../paycloud/test-credentials`; `GET/POST/PATCH /api/admin/paycloud-operations/merchants`; `POST /api/admin/paycloud-operations/reconcile` | **SPA:** [`PayCloudIntegrationPage`](../../apps/admin-web/src/routes/integrations/PayCloudIntegrationPage.tsx). Tenant PayCloud app credentials (live/sandbox), feature-flag snapshot, merchant registry, credential test + reconciliation trigger. AuthZ `ADMIN_SECTION_INTEGRATIONS_DEV`. |
