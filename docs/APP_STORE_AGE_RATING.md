...
|----------|--------|---------------|
| **Parental Controls** | **Yes** | Pre-auth: Login → **Safety & parental controls** (`apps/customer/app/(auth)/safety-and-age.tsx`). Post-auth: Profile → **Safety & parental controls** → **Parental controls — content & safety** (`apps/customer/app/(app)/account-settings/content-and-safety-controls.tsx`). Device auth gates changes when enabled. Teen defaults locked server-side (`apps/web/src/lib/age-assurance/safety-settings.ts`). |
...
> Beautonomi is a beauty and wellness marketplace. Users can browse Home, Search, Shop, Explore, partner profiles, and product pages without an account; booking, checkout, cart, messaging, and account features require sign-in. **Pre-login parental controls:** Login screen → **Safety & parental controls** (explains mechanisms + Age Suitability policy). **Demo account:** email `nomi@ferdose.com`, phone `+27716429097` (or `716429097`), OTP **`246810`**. Minimum age for social features is 13. **Post-login:** Profile tab → **Safety & parental controls** → **Parental controls — content & safety** and **Age assurance & date of birth**. **ATT:** After splash hide, before analytics — delete/reinstall to screen-record. **EULA:** Blocking gate before sign-in; https://www.beautonomi.com/customer/eula Age suitability: https://www.beautonomi.com/age-suitability

### Suggested App Review notes (Provider)

> Beautonomi Partner is a business tool for beauty/wellness providers (18+). **Demo account:** email `buntulink@gmail.com`, phone `+27790624995`, OTP **`246810`**. **EULA:** https://www.beautonomi.com/provider/eula (blocking gate before sign-in; linked on subscription/IAP screens). **UGC:** More → Trust & Safety → Report content / content filters. **ATT:** After splash hide, before Singular. Apple IAP for Growth/Scale subscriptions and paid ads only.
