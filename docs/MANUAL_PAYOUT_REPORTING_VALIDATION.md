...
| RP19 | Subscription / ad billing                                  | `provider_subscription_payment` / `provider_ads_payment` ft             | NOT in provider payout balance       | Not shown in provider net activity           | `subscription_net` / `ads_net` separated      | Visible                                          | Auto (`aggregate-finance-ledger-rows-scenarios.test.ts`) |
