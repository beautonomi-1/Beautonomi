...
- **Create and edit in the CMS**: In **Learning Center → Articles**, use **Add article** to create or **Edit** on any row to change an article. The article editor lets you set category, title, slug, summary, **hero image URL** (`image_url`), **body** (HTML or Markdown), content format/type, status, audience, internal-only, featured order, and publish/schedule times. In the **Vite admin** (`apps/admin-web`), HTML bodies use a **TipTap** editor with **Image** (URL) and **YouTube** toolbar actions for inline media. Legacy **Markdown** articles stay in a plain textarea until you use **convert to HTML**, which runs the same **marked** renderer as the public site and opens the visual editor.
...
- **Body**: HTML or Markdown (set `content_format`). For rich text and media, use HTML in the body. Images, GIFs, and video embedded in the body (e.g. `<img>`, `<video>`, `<iframe>`) are styled to match the article text column. **YouTube**: Use the TipTap **YouTube** button (or paste a watch/shorts URL), or paste an embed iframe from YouTube — allowed iframe hosts on the public site are **YouTube** and **Vimeo**; other iframe `src` values are removed server-side (`sanitizeLearnArticleHtml` on `/learn/article/[slug]`).
