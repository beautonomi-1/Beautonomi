...
| **Payments** | `payment_successful`, `payment_failed`, `payment_pending`, `payment_method_expired`, `partial_payment_received`, `additional_charge_requested`, `refund_processed`, `invoice_generated`, `receipt_sent` | `partial_payment_received`: notifyPartialPayment / gateway partial pay; **`additional_charge_requested`**: `POST …/request-payment` (provider extra line item); mark-paid, refund, send-payment-link, additional-charges |
